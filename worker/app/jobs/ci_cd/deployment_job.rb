...
module CiCd
  ...
  class DeploymentJob < BaseJob
    sidekiq_options queue: "ci_cd_high", retry: 1
    ...
    def fetch_deployment(deployment_id)
      response = api_client.get("/api/v1/internal/ci_cd/deployments/#{deployment_id}")
      response.dig("data", "deployment")
    end

    def update_deployment(deployment_id, **attributes)
      api_client.patch("/api/v1/internal/ci_cd/deployments/#{deployment_id}", {
