...
module CiCd
  ...
  # Queue: ci_cd_webhooks
  # Retry: 3
  class WebhookHandlerJob < BaseJob
    sidekiq_options queue: "ci_cd_webhooks", retry: 3
    ...
    def fetch_webhook_event(webhook_event_id)
      response = api_client.get("/api/v1/internal/ci_cd/webhook_events/#{webhook_event_id}")
      response.dig("data", "webhook_event")
    end

    def update_event_status(webhook_event_id, status, **attributes)
      api_client.patch("/api/v1/internal/ci_cd/webhook_events/#{webhook_event_id}", {
    ...
    def find_matching_pipelines(provider_id, event_type, payload)
      ...
      response = api_client.get("/api/v1/internal/ci_cd/pipelines", {
    ...
    def create_pipeline_run(pipeline, event_type, payload)
      ...
      response = api_client.post("/api/v1/internal/ci_cd/pipeline_runs", {
        pipeline_run: {
