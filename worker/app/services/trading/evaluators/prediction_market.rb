  ...
  module Evaluators
    class PredictionMarket < Base
      ...
      def evaluate
        ...
        net_edge = edge.abs - fee_rate # Deduct fees before evaluating edge

        if edge > edge_threshold && net_edge > 0 && !has_open_position?
          signals << build_signal(
            type: "entry", direction: "long",
            confidence: [net_edge / 0.10, 1.0].min,
            strength: [net_edge / 0.075, 1.0].min,
            reasoning: "Prediction market edge: estimated #{(estimated_prob * 100).round(1)}% vs market #{(market_price * 100).round(1)}% (net edge: #{(net_edge * 100).round(1)}% after fees)",
            indicators: { market_price: market_price, estimated_probability: estimated_prob, edge: edge, net_edge: net_edge, edge_pct: (edge * 100).round(2),
                          limit_order: true, limit_price: (market_price - 0.01).round(4) }
          )
        elsif edge < -edge_threshold && net_edge > 0 && !has_open_position?
          signals << build_signal(
            type: "entry", direction: "short",
            confidence: [net_edge / 0.10, 1.0].min,
            strength: [net_edge / 0.075, 1.0].min,
            reasoning: "Prediction market edge (short): estimated #{(estimated_prob * 100).round(1)}% vs market #{(market_price * 100).round(1)}% (net edge: #{(net_edge * 100).round(1)}% after fees)",
            indicators: { market_price: market_price, estimated_probability: estimated_prob, edge: edge, net_edge: net_edge, edge_pct: (edge * 100).round(2),
                          limit_order: true, limit_price: (market_price + 0.01).round(4) }
          )
        end
      ...
      def estimate_fair_probability(market_price)
        ...
        # Logit-space with dampened CONTRARIAN bias — PM markets mean-revert
        clamped_price = market_price.clamp(0.02, 0.98)
        logit = Math.log(clamped_price / (1.0 - clamped_price))
        adjusted_logit = logit - recent_trend * 0.3 # Contrarian: fade the trend
        estimated = 1.0 / (1.0 + Math.exp(-adjusted_logit))

        # Parity-first edge: if YES + NO ≠ $1.00, that IS the primary signal
        if parity_data && parity_data["yes_price"] && parity_data["no_price"]
          yes_p = parity_data["yes_price"].to_f
          no_p = parity_data["no_price"].to_f
          total = yes_p + no_p
          if total > 0 && (total - 1.0).abs > 0.02
            parity_fair = yes_p / total
            estimated = estimated * 0.2 + parity_fair * 0.8 # 80% parity weight
          end
        end
