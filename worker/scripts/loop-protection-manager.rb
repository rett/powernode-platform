...
class LoopProtectionManager
  def self.status
    ...
    if execution_keys.any?
      puts "\n⚡ Recent Executions (last 10 trackers):"
  ...
  def self.clear_all
    puts "🧹 Clearing all loop protection data..."
    ...
    puts "✅ Cleared #{total_deleted} total keys"
  end

  def self.enable_job(job_pattern)
    puts "🔓 Enabling jobs matching: #{job_pattern}"
    ...
    if disabled_keys.any?
      ...
      puts "✅ Enabled #{deleted} disabled jobs"
    else
      puts "ℹ️  No disabled jobs found matching pattern"
    end
  end

  def self.help
    puts <<~HELP
      🛠️  Loop Protection Manager
