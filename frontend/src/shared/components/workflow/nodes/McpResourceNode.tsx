...
import { Database, Server, OctagonX, FileType } from 'lucide-react';
import { DynamicNodeHandles } from './DynamicNodeHandles';
import { NodeActionsMenu } from '../NodeActionsMenu';
import { useWorkflowContext } from '../WorkflowContext';
import { NodeStatusBadge } from '../ExecutionOverlay';

export const McpResourceNode: React.FC<NodeProps<any>> = ({
  id,
  data,
  selected
}) => {
  ...
  const isConfigured = data.configuration?.mcp_server_id && resourceUri;
  ...
  return (
    <div className={`
      ...
      ${selected ? 'border-[var(--node-mcp-resource-bg)] ring-2 ring-[var(--node-mcp-resource-bg)]/20' : 'border-[var(--node-mcp-resource-bg)]'}
    ...
    `}>
      ...
      <div className="flex items-center gap-3 mb-2">
        <div className="w-8 h-8 bg-[var(--node-mcp-resource-bg)] rounded-lg flex items-center justify-center text-white">
          <Database className="h-4 w-4" />
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-theme-primary truncate text-sm">
            {data.name || 'MCP Resource'}
          </h3>
          <p className="text-xs text-theme-info font-medium truncate">
            {displayName}
          </p>
        </div>
      </div>
...
export default McpResourceNode;
