...
export const AdminSettingsDevelopmentTabPage: React.FC = () => {
  ...
  const handleToggle = async () => {
    ...
    setToggling(true);
    try {
      const response = await adminSettingsApi.updateDevelopmentSettings(!info.enterprise_enabled);
      if (response.success && response.data) {
        setInfo(prev => prev ? { ...prev, enterprise_enabled: response.data.enterprise_enabled } : prev);
      }
    } catch {
      setError('Failed to toggle enterprise mode');
    } finally {
      setToggling(false);
    }
  };
    ...
    return (
      <div className="rounded-lg border border-theme bg-theme-surface p-6">
        <p className="text-sm text-theme-secondary">
          Unable to load enterprise development settings. Ensure the backend is running and enterprise engine is loaded.
        </p>
      </div>
    );
  }

  if (!info.enterprise_installed) {
    return (
      <div className="rounded-lg border border-theme bg-theme-surface p-6">
        <h3 className="text-lg font-medium text-theme-primary">Enterprise Not Loaded</h3>
        <p className="mt-2 text-sm text-theme-secondary">
          The enterprise submodule is present but the engine is not loaded on the backend.
          Restart the backend service to load the enterprise engine.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-theme bg-theme-surface p-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-medium text-theme-primary">Enterprise Mode</h3>
            <p className="mt-1 text-sm text-theme-secondary">
              Toggle enterprise features on or off for development testing.
          ...
          <button
            ...
            className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-theme-interactive-primary focus:ring-offset-2 ${
              info.enterprise_enabled
                ? 'bg-theme-interactive-primary'
                : 'bg-theme-tertiary'
            } ${toggling ? 'opacity-50 cursor-wait' : ''}`}
            role="switch"
            aria-checked={info.enterprise_enabled}
            data-testid="enterprise-mode-toggle"
          >
            <span
              className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                info.enterprise_enabled ? 'translate-x-5' : 'translate-x-0'
              }`}
            />
          </button>
        </div>
        ...
        <div className="mt-4 flex items-center gap-2">
          <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${
            ...
            {info.enterprise_enabled ? 'Enabled' : 'Disabled'}
          </span>
        </div>
      </div>
      ...
      {/* Enterprise Info */}
      <div className="rounded-lg border border-theme bg-theme-surface p-6">
        <h3 className="text-lg font-medium text-theme-primary mb-4">Enterprise Details</h3>
      ...
      {info.feature_flags && info.feature_flags.length > 0 && (
        <div className="rounded-lg border border-theme bg-theme-surface p-6">
          <h3 className="text-lg font-medium text-theme-primary mb-4">Enterprise Feature Flags</h3>
            ...
            {info.feature_flags.map((flag) => (
              <div key={flag.name} className="flex items-center justify-between py-2 border-b border-theme last:border-0">
                <span className="text-sm text-theme-primary font-mono">
                  {flag.name.replace(/^enterprise_/, '').replace(/_/g, ' ')}
                </span>
