...
export const WorkflowAnalyticsContent: React.FC = () => {
  ...
  loadAnalyticsData.current = async () => {
    try {
      ...
      let metricsResponse;
      try {
        metricsResponse = await workflowsApi.getExecutionMetrics(dateStrings.start, dateStrings.end);
      } catch (metricsError) {
      ...
      {analyticsData ? (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card><CardContent className="p-4"><div className="flex items-center justify-between"><div><p className="text-sm text-theme-muted">Total Workflows</p><p className="text-2xl font-bold text-theme-primary">{analyticsData?.statistics?.totalWorkflows || 0}</p></div><BarChart3 className="h-8 w-8 text-theme-info" /></div></CardContent></Card>
            <Card><CardContent className="p-4"><div className="flex items-center justify-between"><div><p className="text-sm text-theme-muted">Active Workflows</p><p className="text-2xl font-bold text-theme-primary">{analyticsData?.statistics?.activeWorkflows || 0}</p></div><Activity className="h-8 w-8 text-theme-success" /></div></CardContent></Card>
            <Card><CardContent className="p-4"><div className="flex items-center justify-between"><div><p className="text-sm text-theme-muted">Total Executions</p><p className="text-2xl font-bold text-theme-primary">{analyticsData?.executionMetrics?.metrics?.totalExecutions || 0}</p></div><CheckCircle className="h-8 w-8 text-theme-success" /></div></CardContent></Card>
            <Card><CardContent className="p-4"><div className="flex items-center justify-between"><div><p className="text-sm text-theme-muted">Success Rate</p><p className="text-2xl font-bold text-theme-primary">{formatPercentage(analyticsData?.executionMetrics?.metrics?.successRate || 0)}</p></div><TrendingUp className="h-8 w-8 text-theme-success" /></div></CardContent></Card>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card><CardContent className="p-4"><div className="flex items-center justify-between"><div><p className="text-sm text-theme-muted">Avg Execution Time</p><p className="text-2xl font-bold text-theme-primary">{formatDuration(analyticsData?.executionMetrics?.metrics?.avgExecutionTime || 0)}</p></div><Clock className="h-8 w-8 text-theme-info" /></div></CardContent></Card>
            <Card><CardContent className="p-4"><div className="flex items-center justify-between"><div><p className="text-sm text-theme-muted">Failed Executions</p><p className="text-2xl font-bold text-theme-primary">{analyticsData?.executionMetrics?.metrics?.failedExecutions || 0}</p></div><XCircle className="h-8 w-8 text-theme-danger" /></div></CardContent></Card>
            <Card><CardContent className="p-4"><div className="flex items-center justify-between"><div><p className="text-sm text-theme-muted">Min Execution Time</p><p className="text-2xl font-bold text-theme-primary">{formatDuration(analyticsData?.executionMetrics?.metrics?.minExecutionTime || 0)}</p></div><TrendingDown className="h-8 w-8 text-theme-success" /></div></CardContent></Card>
            <Card><CardContent className="p-4"><div className="flex items-center justify-between"><div><p className="text-sm text-theme-muted">Max Execution Time</p><p className="text-2xl font-bold text-theme-primary">{formatDuration(analyticsData?.executionMetrics?.metrics?.maxExecutionTime || 0)}</p></div><TrendingUp className="h-8 w-8 text-theme-warning" /></div></CardContent></Card>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              <CardTitle className="flex items-center gap-2"><BarChart3 className="h-5 w-5" />Daily Executions</CardTitle>
              <CardContent>
            ...
            <Card>
              <CardTitle className="flex items-center gap-2"><Activity className="h-5 w-5" />Most Active Users</CardTitle>
              <CardContent>
                <div className="space-y-4">
          ...
          <Card>
            <CardTitle className="flex items-center gap-2"><AlertTriangle className="h-5 w-5" />Optimization Recommendations</CardTitle>
            <CardContent>
              <div className="space-y-4">
                {(analyticsData?.executionMetrics?.metrics?.successRate || 0) < 90 && (
                  <div className="p-4 bg-theme-warning/10 border border-theme-warning/20 rounded-lg">
                    ...
                    <p className="text-sm text-theme-warning">Your workflows have a {formatPercentage(analyticsData?.executionMetrics?.metrics?.successRate || 0)} success rate. Consider reviewing failed executions and improving error handling.</p>
                  </div>
                )}
                {(analyticsData?.executionMetrics?.metrics?.avgExecutionTime || 0) > 300000 && (
                  <div className="p-4 bg-theme-info/10 border border-theme-info/20 rounded-lg">
                    ...
                    <p className="text-sm text-theme-info">Average execution time is {formatDuration(analyticsData?.executionMetrics?.metrics?.avgExecutionTime || 0)}. Consider optimizing workflow logic or using parallel execution mode.</p>
                  </div>
                )}
                {(analyticsData?.statistics?.draftWorkflows || 0) > (analyticsData?.statistics?.activeWorkflows || 0) && (
                  <div className="p-4 bg-theme-accent/10 border border-theme-accent/20 rounded-lg">
                    ...
                    <p className="text-sm text-theme-accent">You have {analyticsData?.statistics?.draftWorkflows || 0} draft workflows. Consider reviewing and publishing useful workflows.</p>
                  </div>
                )}
                {(analyticsData?.executionMetrics?.metrics?.totalExecutions || 0) === 0 && (
                  <div className="p-4 bg-theme-surface border border-theme rounded-lg">
                    ...
                    <p className="text-sm text-theme-tertiary">No workflow executions found in the selected period. Try expanding the date range or execute some workflows to see analytics.</p>
                  </div>
                )}
              </div>
            </CardContent>
