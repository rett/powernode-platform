...
const ImpersonateUserModal: React.FC<ImpersonateUserModalProps> = ({
  ...
  const getRoleColor = (role: string): string => {
    if (role.includes('admin')) return 'bg-theme-error bg-opacity-10 text-theme-error';
    if (role.includes('manager')) return 'bg-theme-success bg-opacity-10 text-theme-success';
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-2 flex flex-wrap gap-1">
                          {user.roles && user.roles.length > 0 ? (
                            ...
                            <span className="text-xs text-theme-tertiary">No roles assigned</span>
                          )}
                          {user.roles && user.roles.length > 2 && (
                            <span className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-theme-surface text-theme-tertiary">
                              +{user.roles.length - 2}
                            </span>
                          )}
                        </div>
