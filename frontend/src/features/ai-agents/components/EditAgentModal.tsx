...
export const EditAgentModal: React.FC<EditAgentModalProps> = ({
  ...
  const loadAgentStats = async () => {
    ...
    try {
      ...
      if (is404) {
        ...
        setAgentStats(fallbackStats);
      } else {
        // For other errors, don't set stats (will hide stats section)
        setAgentStats(null);
      }
    } finally {
  ...
  const getModelOptions = () => {
    ...
    if (!selectedProvider.supported_models || !Array.isArray(selectedProvider.supported_models)) {
      return [];
    }

    if (selectedProvider.supported_models.length === 0) {
      return [];
    }

    return selectedProvider.supported_models.map(model => ({
      value: model.id,
      label: model.name || model.id // Prefer display name, fall back to id
    }));
  };
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <div className="text-lg font-semibold text-theme-success">
                  {agentStats.success_rate || 0}%
                </div>
                <div className="text-xs text-theme-tertiary">Success Rate</div>
              </div>
              <div className="text-center">
                <div className="text-lg font-semibold text-theme-primary">
                  {agentStats.avg_execution_time || 0}ms
                </div>
                <div className="text-xs text-theme-tertiary">Avg Time</div>
              </div>
              <div className="text-center">
                <div className="text-lg font-semibold text-theme-warning">
                  ${agentStats.estimated_total_cost || '0.00'}
                </div>
