...
export const AiOpsDashboard: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {realTimeData && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6 p-4 bg-theme-surface rounded-lg border border-theme">
          <div className="text-center">
            <p className="text-xs text-theme-tertiary uppercase">Requests/sec</p>
            <p className="text-xl font-bold text-theme-primary">
              {realTimeData.current_requests_per_second.toFixed(1)}
            </p>
          </div>
          <div className="text-center">
            <p className="text-xs text-theme-tertiary uppercase">Avg Latency</p>
            <p className="text-xl font-bold text-theme-primary">
              {realTimeData.current_avg_latency_ms.toFixed(0)}ms
            </p>
          </div>
          <div className="text-center">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-semibold text-theme-primary">
                {formatNumber(dashboardData.summary.total_requests)}
              </p>
              <p className="text-xs text-theme-tertiary mt-1">
                {dashboardData.summary.success_rate.toFixed(1)}% success
              </p>
            </div>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-semibold text-theme-primary">
                {dashboardData.summary.avg_latency_ms.toFixed(0)}ms
              </p>
              <p className="text-xs text-theme-tertiary mt-1">
                p95: {dashboardData.summary.p95_latency_ms.toFixed(0)}ms
              </p>
            </div>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-semibold text-theme-primary">
                ${dashboardData.summary.total_cost_usd.toFixed(2)}
              </p>
              <p className="text-xs text-theme-tertiary mt-1">
                This period
              </p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-semibold text-theme-error">
                {formatNumber(dashboardData.summary.failed_requests)}
              </p>
              <p className="text-xs text-theme-tertiary mt-1">
                {((dashboardData.summary.failed_requests / dashboardData.summary.total_requests) * 100).toFixed(1)}% error rate
              </p>
            </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  {healthData.alerts_summary.critical > 0 && (
                    <Badge variant="danger" size="sm">{healthData.alerts_summary.critical} critical</Badge>
                  )}
                  {healthData.alerts_summary.warning > 0 && (
                    <Badge variant="warning" size="sm">{healthData.alerts_summary.warning} warning</Badge>
                  )}
                  {healthData.alerts_summary.info > 0 && (
                    <Badge variant="info" size="sm">{healthData.alerts_summary.info} info</Badge>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="space-y-3">
            {dashboardData.top_workflows.map((workflow, index) => (
              <div key={workflow.id} className="flex items-center justify-between p-3 bg-theme-surface rounded-lg">
                <div className="flex items-center gap-3">
                  <div className="w-6 h-6 bg-theme-info rounded flex items-center justify-center text-white text-xs font-bold">
                    {index + 1}
                  </div>
                  <div>
                ...
                <div className="text-right">
                  <div className="flex items-center gap-1">
                    ...
                    <span className={`font-medium ${
                      workflow.success_rate >= 95 ? 'text-theme-success' : 'text-theme-error'
                    }`}>
                      {workflow.success_rate.toFixed(1)}%
                    </span>
                  </div>
                  <p className="text-xs text-theme-tertiary">
                    {(workflow.avg_duration_ms / 1000).toFixed(1)}s avg
                  </p>
                </div>
              </div>
            ))}
