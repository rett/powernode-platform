...
export const useAppPlans = (appId: string | null | undefined, initialLoad = true): UseAppPlansResult => {
  ...
  const loadPlans = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(error) || 'Failed to load app plans';
  ...
  const createPlan = useCallback(async (data: AppPlanFormData): Promise<AppPlan | null> => {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(error) || 'Failed to create app plan';
  ...
  const updatePlan = useCallback(async (id: string, data: Partial<AppPlanFormData>): Promise<AppPlan | null> => {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(error) || 'Failed to update app plan';
      showNotification(errorMessage, 'error');
      return null;
    } finally {
      setLoading(false);
  ...
  const deletePlan = useCallback(async (id: string): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(error) || 'Failed to delete app plan';
      showNotification(errorMessage, 'error');
      return false;
    } finally {
      setLoading(false);
  ...
  const activatePlan = useCallback(async (id: string): Promise<AppPlan | null> => {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(error) || 'Failed to activate app plan';
      showNotification(errorMessage, 'error');
      return null;
    } finally {
      setLoading(false);
  ...
  const deactivatePlan = useCallback(async (id: string): Promise<AppPlan | null> => {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(error) || 'Failed to deactivate app plan';
      showNotification(errorMessage, 'error');
      return null;
    } finally {
      setLoading(false);
