...
import { defaultNavigationConfig, adminNavigationOverrides } from '../config/navigation';
...
export const NavigationProvider: React.FC<NavigationProviderProps> = ({ 
  ...
  const buildNavigationConfig = useCallback((): NavigationConfig => {
    ...
    // Add admin sections if user has admin access
    if (isAdmin && adminNavigationOverrides.sections) {
  ...
  // Permission checker
  const hasPermission = useCallback((permissions?: string[], roles?: string[]): boolean => {
    return hasAccess(user, permissions, roles);
  }, [user]);
