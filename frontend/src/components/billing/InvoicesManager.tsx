...
import { invoicesApi, Invoice, InvoiceFilters, InvoiceStats } from '../../services/invoicesApi';
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {(invoice.status === 'open' || invoice.status === 'overdue') && (
                          <>
                            <button
                              onClick={() => handleAction('retry', invoice.id)}
                              disabled={actionLoading[invoice.id]}
                              className="p-2 text-theme-secondary hover:text-theme-primary transition-colors disabled:opacity-50"
                              title="Retry Payment"
                            >
                        ...
                        {(invoice.status === 'draft' || invoice.status === 'open') && (
                          <button
                            onClick={() => handleAction('void', invoice.id, { reason: 'Manual void' })}
                            disabled={actionLoading[invoice.id]}
