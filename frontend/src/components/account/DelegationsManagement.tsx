...
export const DelegationsManagement: React.FC = () => {
  ...
  const getStatusColor = (delegation: Delegation) => {
    if (delegation.status === 'revoked') return 'text-red-600 bg-red-50';
    if (delegation.is_expired) return 'text-orange-600 bg-orange-50';
    if (delegation.status === 'inactive') return 'text-gray-600 bg-gray-50';
    if (delegation.is_active) return 'text-green-600 bg-green-50';
  ...
  const getRoleColor = (roleName: string) => {
    switch (roleName.toLowerCase()) {
      ...
      case 'owner': return 'bg-theme-success bg-opacity-10 text-theme-success';
