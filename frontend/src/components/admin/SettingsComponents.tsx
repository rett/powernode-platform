...
export const ToggleSwitch: React.FC<ToggleSwitchProps> = ({
  ...
  const getVariantClass = () => {
    switch (variant) {
      case 'success': return 'peer-checked:bg-theme-success';
      case 'warning': return 'peer-checked:bg-theme-warning';
      case 'error': return 'peer-checked:bg-theme-error';
      case 'primary': return 'peer-checked:bg-theme-interactive-primary';
      default: return 'peer-checked:bg-theme-success';
    }
  };

  const sizeClass = size === 'sm' ? 'w-9 h-5' : 'w-11 h-6';
  const toggleSizeClass = size === 'sm' ? 'after:h-4 after:w-4' : 'after:h-5 after:w-5';
  ...
  return (
    <label className="relative inline-flex items-center cursor-pointer">
      <input
        ...
        disabled={disabled}
      />
      <div className={`${sizeClass} bg-theme-surface-secondary rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-theme after:border after:rounded-full ${toggleSizeClass} after:transition-all ${getVariantClass()} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`} />
    </label>
  );
};
