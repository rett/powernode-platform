...
# Usage: ./scripts/validate.sh [--skip-tests] [--skip-ts] [--skip-patterns]
...
for arg in "$@"; do
  case "$arg" in
    ...
    --help)
      echo "Usage: ./scripts/validate.sh [--skip-tests] [--skip-ts] [--skip-patterns]"
      echo ""
...
if [[ "$SKIP_TESTS" == "false" ]]; then
  echo -e "${BLUE}[1/3] Running backend specs...${NC}"
...
if [[ "$SKIP_TS" == "false" ]]; then
  echo -e "${BLUE}[2/3] Running TypeScript type check...${NC}"
...
if [[ "$SKIP_PATTERNS" == "false" ]]; then
  echo -e "${BLUE}[3/3] Running pattern validation...${NC}"
