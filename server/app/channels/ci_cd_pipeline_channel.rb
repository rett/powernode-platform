...
class CiCdPipelineChannel < ApplicationCable::Channel
  def subscribed
    ...
    if current_user && authorized_for_account?(account_id)
      if pipeline_id.present?
        ...
        stream_from "ci_cd_pipeline_#{pipeline_id}"
        Rails.logger.info "User #{current_user.id} subscribed to pipeline #{pipeline_id}"
      else
        # Subscribe to all pipelines for account
        stream_from "ci_cd_account_#{account_id}"
        Rails.logger.info "User #{current_user.id} subscribed to all CI/CD updates for account #{account_id}"
      end

      transmit({
        type: "subscribed",
        message: "Connected to CI/CD pipeline updates",
        pipeline_id: pipeline_id,
        timestamp: Time.current.iso8601
      })
    else
      Rails.logger.warn "Unauthorized CI/CD subscription attempt by user #{current_user&.id}"
      reject
    end
  end

  def unsubscribed
    Rails.logger.info "User #{current_user&.id} unsubscribed from CI/CD updates"
  end
  ...
  class << self
    ...
    def broadcast_to_pipeline(pipeline, data)
      ActionCable.server.broadcast("ci_cd_pipeline_#{pipeline.id}", data)
    end

    def broadcast_to_account(account, data)
      ActionCable.server.broadcast("ci_cd_account_#{account.id}", data)
    end
