  ...
  module V1
    module CiCd
      class PipelinesController < ApplicationController
        ...
        # GET /api/v1/ci_cd/pipelines
        def index
          pipelines = current_user.account.ci_cd_pipelines
          ...
          render_success({
            ...
            meta: {
              ...
              active_count: current_user.account.ci_cd_pipelines.where(is_active: true).count,
              total_runs: current_user.account.ci_cd_pipelines.joins(:runs).count
            }
          })
        ...
        # POST /api/v1/ci_cd/pipelines
        def create
          pipeline = current_user.account.ci_cd_pipelines.new(pipeline_params)
        ...
        # PATCH/PUT /api/v1/ci_cd/pipelines/:id
        def update
        ...
        # DELETE /api/v1/ci_cd/pipelines/:id
        def destroy
        ...
        # POST /api/v1/ci_cd/pipelines/:id/trigger
        # Params:
        ...
        # GET /api/v1/ci_cd/pipelines/:id/export_yaml
        def export_yaml
        ...
        # POST /api/v1/ci_cd/pipelines/:id/duplicate
        def duplicate
        ...
        def set_pipeline
          @pipeline = current_user.account.ci_cd_pipelines.find(params[:id])
        rescue ActiveRecord::RecordNotFound
          render_error("Pipeline not found", status: :not_found)
        end

        def require_read_permission
          return if current_user.has_permission?("ci_cd.pipelines.read")

          render_error("Insufficient permissions to view pipelines", status: :forbidden)
        end

        def require_write_permission
          return if current_user.has_permission?("ci_cd.pipelines.write")
