...
# UnifiedMonitoringService - Single consolidated monitoring service
#
# Replaces:
# - AiMonitoringService
...
class UnifiedMonitoringService
  ...
  def workflow_summary(workflow, time_range)
    runs = workflow.workflow_runs.where("created_at >= ?", time_range.ago)
  ...
  def provider_is_healthy?(provider)
    ...
    recent_executions = Ai::AgentExecution.where(ai_provider: provider)
  ...
  def count_provider_executions(provider, time_range)
    Ai::AgentExecution.where(ai_provider: provider)
                   .where("created_at >= ?", time_range.ago)
                   .count
  end

  def calculate_provider_success_rate(provider, time_range)
    executions = Ai::AgentExecution.where(ai_provider: provider)
                                .where("created_at >= ?", time_range.ago)
  ...
  def count_agent_executions(agent, time_range)
    agent.agent_executions.where("created_at >= ?", time_range.ago).count
  end

  def calculate_agent_success_rate(agent, time_range)
    executions = agent.agent_executions.where("created_at >= ?", time_range.ago)
  ...
  def aggregate_workflow_metrics(workflows, time_range)
    all_runs = Ai::WorkflowRun.where(ai_workflow: workflows)
                           .where("created_at >= ?", time_range.ago)
