...
class GitProviderManagementService
  ...
  class << self
    ...
    def sync_single_repository(credential, repo_data, options = {})
      ...
      repo = credential.git_repositories.find_or_initialize_by(
    ...
    def setup_default_providers
      providers_config.each do |config|
        GitProvider.find_or_create_by!(slug: config[:slug]) do |provider|
    ...
    def validate_credential_params(provider, params)
      ...
      unless GitProviderCredential::AUTH_TYPES.include?(params[:auth_type])
    ...
    def test_and_update_credential(credential)
      result = GitProviderTestService.new(credential).test_connection
