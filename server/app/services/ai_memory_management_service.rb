...
class AiMemoryManagementService
  ...
  class << self
    ...
    def cleanup_expired_entries
      expired = AiContextEntry.expired.where(archived_at: nil)
    ...
    def generate_missing_embeddings(context: nil, batch_size: 100)
      scope = AiContextEntry.active.where(embedding: nil).where.not(content_text: nil)
      scope = scope.where(ai_persistent_context: context) if context.present?
    ...
    def memory_stats(account:)
      contexts = AiPersistentContext.where(account: account)
      entries = AiContextEntry.joins(:ai_persistent_context)
    ...
    def export_memories(account:, format: :json, include_embeddings: false)
      contexts = AiPersistentContext.where(account: account).includes(:ai_context_entries)
    ...
    def find_or_create_context(account, context_data)
      AiPersistentContext.find_or_create_by!(
        account: account,
    ...
    def replace_context(account, context_data)
      existing = AiPersistentContext.find_by(
        account: account,
        name: context_data[:name],
        context_type: context_data[:context_type]
      )
    ...
    def create_new_context(account, context_data)
      ...
      while AiPersistentContext.exists?(account: account, name: name)
        name = "#{context_data[:name]} (#{counter})"
        counter += 1
      end

      AiPersistentContext.create!(
        account: account,
        name: name,
        context_type: context_data[:context_type],
