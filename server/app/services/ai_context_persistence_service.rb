...
class AiContextPersistenceService
  ...
  class << self
    ...
    def create_context(account:, attributes:, created_by: nil)
      context = AiPersistentContext.new(
        account: account,
    ...
    def find_context(account:, context_id:, accessor: nil)
      context = AiPersistentContext.find_by(id: context_id, account: account)
    ...
    def list_contexts(account:, filters: {}, page: 1, per_page: 20)
      scope = AiPersistentContext.where(account: account)
    ...
    def add_entry(context:, attributes:, accessor: nil)
      ...
      entry = context.ai_context_entries.new(
        ...
        ai_agent_id: accessor.is_a?(AiAgent) ? accessor.id : attributes[:agent_id],
    ...
    def get_agent_memory(account:, agent:, create_if_missing: true)
      context = AiPersistentContext.find_by(
        account: account,
