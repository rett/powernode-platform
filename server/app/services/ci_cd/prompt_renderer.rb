...
module CiCd
  # CI/CD-specific prompt renderer that extends the shared prompt renderer
  ...
  class PromptRenderer
    ...
    class << self
      ...
      def render_by_id(template_id, account:, variables: {}, context: {})
        template = account.shared_prompt_templates.for_cicd.find(template_id)
      ...
      def render_by_slug(slug, account:, variables: {}, context: {})
        template = account.shared_prompt_templates.for_cicd.find_by!(slug: slug)
        new(template, variables: variables, context: context).render
      rescue ActiveRecord::RecordNotFound
