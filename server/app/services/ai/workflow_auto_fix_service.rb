...
#
class Ai::WorkflowAutoFixService
  ...
  def fix_all
    ...
    validation_service = Ai::WorkflowValidationService.new(workflow)
  ...
  def fix_missing_start_node
    nodes = workflow.nodes.includes(:target_edges)
  ...
  def fix_missing_timeout(issue)
    ...
    node = workflow.nodes.find_by(id: issue[:node_id])
  ...
  def fix_orphaned_node(issue)
    ...
    orphaned_node = workflow.nodes.find_by(id: issue[:node_id])
    return log_error("Node not found: #{issue[:node_id]}") unless orphaned_node

    # Find start node or first node
    start_node = workflow.nodes.find_by(is_start_node: true) ||
  ...
  def log_error(message)
    @errors << message
    Rails.logger.error("[Ai::WorkflowAutoFixService] #{message}")
    false
  end
end
