...
# Ai::AgentOrchestrationService - Main facade for AI workflow orchestration
#
...
class Ai::AgentOrchestrationService
  ...
  def initialize(workflow = nil, account: nil, user: nil)
    ...
    begin
      @load_balancer = Ai::ProviderLoadBalancerService.new(@account, strategy: "cost_optimized") if @account
      @error_recovery = Ai::ErrorRecoveryService.new(@account, @execution_context) if @account
