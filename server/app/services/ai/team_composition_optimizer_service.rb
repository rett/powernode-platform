...
module Ai
  class TeamCompositionOptimizerService
    ...
    # Analyze a team's composition for gaps, redundancies, and coverage
    def analyze(team)
      members = team.ai_agent_team_members.includes(:ai_agent)
      agents = members.map(&:ai_agent).compact
    ...
    # Recommend agents to add or remove based on task description
    def recommend_agents(team, task_description)
      analyzer = Discovery::TaskAnalyzerService.new(account: account)
      analysis = analyzer.analyze(task_description)
      current_analysis = analyze(team)

      add_recommendations = []
      remove_recommendations = []

      # Recommend additions for gaps
      ...
      # Recommend removals for redundancies
    ...
    # Auto-optimize team composition with guardrail checks
    def auto_optimize(team)
      analysis = analyze(team)
