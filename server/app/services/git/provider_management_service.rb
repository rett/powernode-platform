    ...
    class << self
      ...
      def setup_default_providers
        providers_config.each do |config|
          Git::Provider.find_or_create_by!(slug: config[:slug]) do |provider|
      ...
      def validate_credential_params(provider, params)
        ...
        unless Git::ProviderCredential::AUTH_TYPES.include?(params[:auth_type])
      ...
      def test_and_update_credential(credential)
        result = Git::ProviderTestService.new(credential).test_connection
...
GitProviderManagementService = Git::ProviderManagementService unless defined?(GitProviderManagementService)
