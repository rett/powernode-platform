  ...
  class << self
    ...
    def find_template(identifier)
      template = if uuid?(identifier)
        IntegrationTemplate.find_by(id: identifier)
      else
        IntegrationTemplate.find_by(slug: identifier)
      end
    ...
    def create_template(attributes)
      template = IntegrationTemplate.new(attributes)
    ...
    def list_instances(account:, filters: {}, page: 1, per_page: 20)
      scope = IntegrationInstance.where(account: account)

      scope = scope.where(status: filters[:status]) if filters[:status].present?
      scope = scope.joins(:integration_template)
                   .where(integration_templates: { integration_type: filters[:type] }) if filters[:type].present?

      scope.includes(:integration_template, :integration_credential)
    ...
    def find_instance(account:, instance_id:)
      instance = IntegrationInstance
        ...
        .includes(:integration_template, :integration_credential)
    ...
    def install_template(account:, template_identifier:, attributes: {}, created_by: nil)
      ...
      instance = IntegrationInstance.new(
        account: account,
      ...
      if attributes[:credential_id].present?
        credential = IntegrationCredential.find_by(id: attributes[:credential_id], account: account)
        raise ValidationError, "Invalid credential" unless credential

        instance.integration_credential = credential
      end
    ...
    def uninstall_instance(account:, instance_id:)
      ...
      ActiveRecord::Base.transaction do
        instance.integration_template.decrement!(:usage_count)
        instance.destroy!
      end
    ...
    def create_credential(account:, attributes:, created_by: nil)
      credential = IntegrationCredential.new(
        account: account,
        created_by_user: created_by,
      ...
      ActiveRecord::Base.transaction do
        ...
        if attributes[:credentials].present?
          IntegrationCredentialEncryptionService.encrypt(credential, attributes[:credentials])
        end
      end
    ...
    def update_credential(account:, credential_id:, attributes:)
      credential = IntegrationCredential.find_by(id: credential_id, account: account)
    ...
    def delete_credential(account:, credential_id:)
      ...
      if IntegrationInstance.where(integration_credential: credential).exists?
    ...
    def search_templates(query:, filters: {}, page: 1, per_page: 20)
      scope = IntegrationTemplate.where(is_public: true)
    ...
    def integration_types
      IntegrationTemplate::INTEGRATION_TYPES
    end
    ...
    def generate_slug(name, account)
      ...
      while IntegrationInstance.exists?(account: account, slug: slug)
    ...
    def build_executor(instance)
      IntegrationExecutionService.build_executor(instance)
    end
  end
end
