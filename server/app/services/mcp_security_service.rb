...
# Service for MCP security hardening
...
class McpSecurityService
  ...
  # These are common runtimes used for MCP servers
  ALLOWED_COMMANDS = %w[
    npx
    node
    python
  ...
  # Additional commands that can be enabled by configuration
  EXTENDED_COMMANDS = %w[
    uvx
    uv
    pipx
  ...
  # Variables with these prefixes are allowed through
  ...
  # Explicitly allowed individual environment variables
  ALLOWED_ENV_VARS = %w[
    PATH
    HOME
    USER
  ...
  class << self
    ...
    # @param allow_extended [Boolean] Whether to allow extended commands
    # @raise [CommandNotAllowedError] If command is not allowed
    def validate_command!(command, allow_extended: false)
      return if command.blank?

      # Extract base command (first word, handling full paths)
    ...
    # @param env [Hash] Environment variables to sanitize
    # @param strict [Boolean] If true, only allow explicit whitelist; if false, allow common vars
    # @return [Hash] Sanitized environment variables
    ...
    def env_allowed?(key, strict: false)
      ...
      # In non-strict mode, allow additional common vars
      !strict
    end

    # Get list of forbidden environment variables found in env hash
    def forbidden_env_vars(env)
      return [] if env.blank?

      env.keys.map(&:to_s).map(&:upcase) & FORBIDDEN_ENV_VARS
    end

    # Validate environment and raise if forbidden vars present
    def validate_environment!(env)
      forbidden = forbidden_env_vars(env)
    ...
    def extract_base_command(command)
      ...
      # Handle 'env' command prefix (e.g., "env VAR=value command")
    ...
    def allowed_commands(allow_extended)
      if allow_extended
        ALLOWED_COMMANDS + EXTENDED_COMMANDS
      else
        ALLOWED_COMMANDS
      end
    end
    ...
    def validate_command_arguments!(command)
      ...
      dangerous_patterns = [
        ...
        /\|\s*\w+/, # Pipe to command (allow if followed by space)
