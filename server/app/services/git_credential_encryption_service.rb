...
class GitCredentialEncryptionService
  ...
  class << self
    def encrypt(credentials_hash)
      ...
      Rails.logger.error "GitCredentialEncryptionService encrypt error: #{e.message}"
    ...
    def decrypt(encrypted_data, key_id = nil)
      ...
      Rails.logger.error "GitCredentialEncryptionService decrypt error: Invalid message"
      raise DecryptionError, "Failed to decrypt credentials: Invalid encryption"
    rescue JSON::ParserError => e
      Rails.logger.error "GitCredentialEncryptionService decrypt error: Invalid JSON"
      raise DecryptionError, "Failed to decrypt credentials: Invalid data format"
    rescue StandardError => e
      Rails.logger.error "GitCredentialEncryptionService decrypt error: #{e.message}"
