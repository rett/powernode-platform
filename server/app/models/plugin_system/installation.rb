...
module PluginSystem
  ...
  class Installation < ApplicationRecord
    ...
    def register_plugin_resources
      ...
      if plugin.provider?
        PluginSystem::ProviderRegistryService.new(account: account).register_provider_plugin(self)
      end

      if plugin.workflow_node?
        PluginSystem::NodeRegistryService.new(account: account).register_node_plugin(self)
      end
    ...
    def unregister_plugin_resources
      if plugin.provider?
        PluginSystem::ProviderRegistryService.new(account: account).unregister_provider_plugin(self)
      end

      if plugin.workflow_node?
        PluginSystem::NodeRegistryService.new(account: account).unregister_node_plugin(self)
      end
    rescue StandardError => e
