...
module PluginSystem
  ...
  class Definition < ApplicationRecord
    ...
    belongs_to :source_marketplace, class_name: "PluginSystem::Marketplace", optional: true

    has_many :installations, class_name: "PluginSystem::Installation", foreign_key: :plugin_id, dependent: :destroy
    has_one :ai_provider_plugin, class_name: "Ai::ProviderPlugin", foreign_key: :plugin_id, dependent: :destroy
    has_many :workflow_node_plugins, class_name: "WorkflowNodePlugin", foreign_key: :plugin_id, dependent: :destroy
    ...
    # Alias for consistency with original method name
    alias_method :provider?, :ai_provider?
    ...
    def install_for_account(target_account, user, configuration = {})
      PluginSystem::InstallationService.new.install_plugin(
        self,
        target_account,
        user,
        configuration
    ...
    def installed_for?(target_account)
      installations.where(account: target_account, status: "active").exists?
    end

    def installation_for(target_account)
      installations.find_by(account: target_account)
    end
