...
module CiCd
  ...
  class PipelineStep < ApplicationRecord
    ...
    has_many :executions, class_name: "CiCd::StepExecution", foreign_key: :ci_cd_pipeline_step_id, dependent: :destroy
    ...
    def handler_class
      "CiCd::StepHandlers::#{step_type.camelize}Handler".constantize
    rescue NameError
      CiCd::StepHandlers::CustomHandler
    end

    def should_run?(previous_step_outputs)
      return true if condition.blank?

      evaluator = CiCd::ConditionEvaluator.new(condition, previous_step_outputs)
      evaluator.evaluate
