...
module CiCd
  ...
  class StepExecution < ApplicationRecord
    ...
    # Associations
    # ============================================
    belongs_to :pipeline_run, class_name: "CiCd::PipelineRun", foreign_key: :ci_cd_pipeline_run_id
    belongs_to :pipeline_step, class_name: "CiCd::PipelineStep", foreign_key: :ci_cd_pipeline_step_id

    has_many :approval_tokens, class_name: "CiCd::StepApprovalToken", foreign_key: :step_execution_id, dependent: :destroy
    ...
    scope :ordered, -> { joins(:pipeline_step).order("ci_cd_pipeline_steps.position ASC") }
    ...
    def enqueue_execution
      WorkerJobService.enqueue_ci_cd_step_execution(id)
    end
    ...
    def broadcast_approval_required
      CiCdPipelineChannel.broadcast_step_update(
