...
class GitProviderCredential < ApplicationRecord
  ...
  scope :for_provider, ->(provider) { where(git_provider: provider) }
  ...
  def make_default!
    transaction do
      account.git_provider_credentials
             .where(git_provider: git_provider, is_default: true)
             .where.not(id: id)
  ...
  def test_connection
    ...
    begin
      GitProviderTestService.new(self).test_connection
  ...
  def usage_summary
    {
      ...
      repositories_count: git_repositories.count,
  ...
  def decrypt_credentials
    ...
    if Rails.env.test?
      ...
      GitCredentialEncryptionService.decrypt(encrypted_credentials, encryption_key_id)
    end
  rescue StandardError => e
  ...
  def encrypt_credentials(credentials_hash)
    ...
    if Rails.env.test?
      ...
      GitCredentialEncryptionService.encrypt(credentials_hash)
    end
  end

  def current_encryption_key_id
    Rails.env.test? ? "test_key" : GitCredentialEncryptionService.current_key_id
  end
  ...
  def prevent_destroy_if_default_and_only
    if is_default? && account.git_provider_credentials.for_provider(git_provider).count == 1
  ...
  def set_as_default_if_first
    return if account.git_provider_credentials.for_provider(git_provider).count > 1
