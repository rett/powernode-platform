...
module Devops
  class GitProviderCredential < ApplicationRecord
    # Table name (using git_ prefix, not devops_)
    self.table_name = "git_provider_credentials"
    ...
    # Associations
    belongs_to :provider, class_name: "Devops::GitProvider", foreign_key: "git_provider_id"
    belongs_to :account
    belongs_to :user, optional: true
    has_many :repositories, class_name: "Devops::GitRepository", foreign_key: "git_provider_credential_id", dependent: :destroy
    ...
    scope :for_provider, ->(provider) { where(git_provider_id: provider.is_a?(Devops::GitProvider) ? provider.id : provider) }
    ...
    def test_connection
      ...
      begin
        Devops::Git::ProviderTestService.new(self).test_connection
    ...
    def decrypt_credentials
      ...
      if Rails.env.test?
        ...
        Security::CredentialEncryptionService.decrypt(encrypted_credentials, namespace: "git")
      end
    rescue StandardError => e
    ...
    def encrypt_credentials(credentials_hash)
      ...
      if Rails.env.test?
        ...
        Security::CredentialEncryptionService.encrypt(credentials_hash, namespace: "git")
      end
    end

    def current_encryption_key_id
      Rails.env.test? ? "test_key" : Security::CredentialEncryptionService.current_key_id("git")
    end
    ...
    def ensure_single_default
      ...
      Devops::GitProviderCredential.where(
        account: account,
