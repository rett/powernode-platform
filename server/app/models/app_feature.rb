...
class AppFeature < ApplicationRecord
  ...
  # Associations
  belongs_to :app
  has_many :dependent_features, class_name: "AppFeature", foreign_key: "dependency_id"
  ...
  def dependency_features
    return AppFeature.none if dependencies.empty?

    app.app_features.where(slug: dependencies)
  end
  ...
  def dependent_features_list
    app.app_features.where("dependencies @> ?", [ slug ].to_json)
  end
  ...
  # Usage tracking
  def used_in_plans
    app.app_plans.where("features @> ?", [ slug ].to_json)
  end
  ...
  def subscriber_count
    used_in_plans.joins(:app_subscriptions)
  ...
  def generate_slug
    ...
    while app.app_features.exists?(slug: candidate_slug)
  ...
  def validate_dependencies_exist
    ...
    existing_features = app.app_features.where.not(id: id).pluck(:slug)
  ...
  def would_create_circular_dependency?
    ...
    all_deps = {}
    app.app_features.where.not(id: id).each do |feature|
