...
module Marketplace
  class Review < ApplicationRecord
    ...
    belongs_to :account
    has_many :helpfulness_votes, class_name: "Review::HelpfulnessVote", foreign_key: "app_review_id", dependent: :destroy
    has_many :responses, class_name: "Review::Response", foreign_key: "app_review_id", dependent: :destroy
    has_many :media_attachments, class_name: "Review::MediaAttachment", foreign_key: "app_review_id", dependent: :destroy
    has_many :moderation_actions, class_name: "Review::ModerationAction", foreign_key: "app_review_id", dependent: :destroy
    has_many :approved_responses, -> { approved }, class_name: "Review::Response", foreign_key: "app_review_id"
    ...
    scope :with_media, -> { joins(:media_attachments).distinct }
    ...
    def verified_purchase?
      Marketplace::Subscription.exists?(account: account, app: app)
    end

    def long_term_user?
      subscription = Marketplace::Subscription.find_by(account: account, app: app)
    ...
    def reviewer_other_reviews(limit = 3)
      Marketplace::Review.where(account: account)
                         .where.not(id: id)
                         .includes(:app)
    ...
    def review_media_attachments
      media_attachments
    end

    def review_moderation_actions
      moderation_actions
    end
    ...
    def check_verification_status
      self.verified_purchase = Marketplace::Subscription.exists?(account: account, app: app)
      save! if changed?
    end
