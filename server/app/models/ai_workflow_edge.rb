...
class AiWorkflowEdge < ApplicationRecord
  ...
  def validate_source_and_target_exist
    ...
    workflow_node_ids = ai_workflow.ai_workflow_nodes.pluck(:node_id)
  ...
  def evaluate_expression(expression, context)
    ...
    # In production, consider using a sandboxed expression evaluator
    ...
    # Very basic expression evaluation - expand this for production use
  ...
  def extract_variables_from_condition(condition_hash)
    ...
    if condition_hash['expression'].present?
      # Extract variables from expression (variables are in ${var} or $var format)
  ...
  def check_for_cycle(current_node_id, visited)
    ...
    outgoing_edges = ai_workflow.ai_workflow_edges.where(source_node_id: current_node_id)
  ...
  def calculate_path_length_to_end(current_node_id, visited, depth = 0)
    ...
    current_node = ai_workflow.ai_workflow_nodes.find_by(node_id: current_node_id)
