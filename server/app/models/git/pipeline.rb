...
module Git
  class Pipeline < ApplicationRecord
    ...
    # Associations
    belongs_to :repository, class_name: "Git::Repository", foreign_key: "git_repository_id"
    belongs_to :account
    has_one :credential, through: :repository, source: :credential
    has_one :provider, through: :repository, source: :provider
    has_many :jobs, class_name: "Git::PipelineJob", foreign_key: "git_pipeline_id", dependent: :destroy
    has_many :approvals, class_name: "Git::PipelineApproval", foreign_key: "git_pipeline_id", dependent: :destroy

    # Delegations
    delegate :provider_type, :full_name, to: :repository, prefix: :repository, allow_nil: true
    ...
    def git_pipeline_jobs
      jobs
    end

    def git_pipeline_approvals
      approvals
    end
    ...
    def sync_job(job_data)
      job = jobs.find_or_initialize_by(external_id: job_data["id"].to_s)
