...
module Ai
  class Workflow
    module StateChecks
      ...
      def can_execute?
        (active? || paused?) && has_valid_structure? && workflow_nodes.any?
      end
      ...
      def can_delete?
        ...
        return false if workflow_runs.where(status: %w[pending running initializing]).exists?

        # Don't allow deletion if there are recent executions (within last 5 minutes)
        return false if workflow_runs.where("created_at > ?", 5.minutes.ago).exists?
