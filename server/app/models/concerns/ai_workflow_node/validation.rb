...
module AiWorkflowNode::Validation
  ...
  def validate_consolidated_node_action
    ...
    when "kb_article"
      ...
      unless AiWorkflowNode::KB_ARTICLE_ACTIONS.include?(action)
        errors.add(:configuration, "action must be one of: #{AiWorkflowNode::KB_ARTICLE_ACTIONS.join(', ')}")
      end
    when "page"
      action = configuration["action"]
      unless AiWorkflowNode::PAGE_ACTIONS.include?(action)
        errors.add(:configuration, "action must be one of: #{AiWorkflowNode::PAGE_ACTIONS.join(', ')}")
      end
    when "mcp_operation"
      operation_type = configuration["operation_type"]
      unless AiWorkflowNode::MCP_OPERATION_TYPES.include?(operation_type)
        errors.add(:configuration, "operation_type must be one of: #{AiWorkflowNode::MCP_OPERATION_TYPES.join(', ')}")
      end
    end
  end
