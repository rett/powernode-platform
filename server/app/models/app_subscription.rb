...
class AppSubscription < ApplicationRecord
  ...
  scope :by_plan, ->(plan) { where(app_plan: plan) }
  ...
  def usage_within_limits?
    app_plan.limits.all? do |limit_name, limit_value|
  ...
  def remaining_quota(limit_name)
    limit_value = app_plan.get_limit(limit_name)
  ...
  def enabled_features
    app.app_features.where(slug: app_plan.features)
  end

  def feature_enabled?(feature_slug)
    app_plan.feature_enabled?(feature_slug)
  end

  def has_permission?(permission)
    app_plan.has_permission?(permission)
  end
  ...
  def calculate_next_billing_date
    ...
    case app_plan.billing_interval
    when "monthly"
  ...
  def grant_app_permissions
    ...
    app_plan.permissions.each do |permission|
  ...
  def log_subscription_created
    Rails.logger.info "App subscription created: Account #{account_id} subscribed to #{app.name} (Plan: #{app_plan.name})"
  end
