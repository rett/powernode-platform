...
class AppEndpoint < ApplicationRecord
  # Associations
  belongs_to :app
  has_many :app_endpoint_calls, dependent: :destroy
  ...
  def average_response_time
    app_endpoint_calls.where.not(response_time_ms: nil).average(:response_time_ms)&.to_f&.round(2) || 0
  end
  ...
  def generate_slug
    ...
    while app&.app_endpoints&.exists?(slug: potential_slug)
      counter += 1
