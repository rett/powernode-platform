  ...
  class Message < ApplicationRecord
    ...
    def display_role
      ...
      when "user"
        user&.full_name || "User"
      when "assistant"
        conversation.agent&.name || conversation.provider.name
      when "system"
        "System"
      else
        role.humanize
    ...
    def update_conversation_tokens_and_cost
      ...
      conversation.increment!(:total_tokens, token_count)
      conversation.increment!(:total_cost, cost_usd) if cost_usd > 0
    end
