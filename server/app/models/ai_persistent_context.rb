...
class AiPersistentContext < ApplicationRecord
  # ==================== Concerns ====================
  include Auditable
  ...
  # ==================== Associations ====================
  belongs_to :account
  belongs_to :ai_agent, optional: true  # null = account-wide context
  belongs_to :created_by_user, class_name: "User", optional: true

  has_many :ai_context_entries, dependent: :destroy
  has_many :ai_context_access_logs, dependent: :destroy
  ...
  def clear_data!
    ...
    self.last_modified_at = Time.current
    save!
    ai_context_entries.destroy_all
  end
  ...
  def create_snapshot
    {
      ...
      entries: ai_context_entries.map(&:entry_snapshot),
      metadata: {
    ...
    transaction do
      ...
      if snapshot["entries"].present?
        ...
        snapshot["entries"].each do |entry_data|
          ai_context_entries.create!(entry_data.except("id", "created_at", "updated_at"))
        end
      end
    end
  end
  ...
  def agent_required_for_agent_scope
    if scope == "agent" && ai_agent_id.blank?
      errors.add(:ai_agent, "is required for agent-scoped contexts")
    end
  end
