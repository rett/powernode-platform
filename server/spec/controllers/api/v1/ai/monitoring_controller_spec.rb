...
RSpec.describe Api::V1::Ai::MonitoringController, type: :controller do
  ...
  describe 'GET #health' do
    before do
      ...
      allow(WorkflowCircuitBreakerManager).to receive(:health_summary).and_return({
        total: 5,
        healthy: 5,
        degraded: 0,
        unhealthy: 0
  ...
  describe 'GET #circuit_breakers_index' do
    before do
      allow(WorkflowCircuitBreakerManager).to receive(:all_states).and_return([
  ...
  describe 'GET #circuit_breaker_show' do
    ...
    before do
      allow(WorkflowCircuitBreakerManager).to receive(:get_breaker).with('provider_openai').and_return(mock_breaker)
    end
    ...
    context 'with valid permissions' do
      ...
      it 'returns not found for unknown circuit breaker' do
        allow(WorkflowCircuitBreakerManager).to receive(:get_breaker).with('unknown').and_return(nil)
  ...
  describe 'POST #circuit_breaker_reset' do
    ...
    before do
      allow(WorkflowCircuitBreakerManager).to receive(:get_or_create_breaker).with('provider_openai').and_return(mock_breaker)
    end
  ...
  describe 'POST #circuit_breakers_reset_all' do
    before do
      allow(WorkflowCircuitBreakerManager).to receive(:reset_all!)
      allow(WorkflowCircuitBreakerManager).to receive(:health_summary).and_return({
        total: 5,
        healthy: 5,
        degraded: 0,
    ...
    context 'with valid permissions' do
      ...
      it 'resets all circuit breakers' do
        expect(WorkflowCircuitBreakerManager).to receive(:reset_all!)
  ...
  describe 'GET #circuit_breakers_category' do
    before do
      allow(WorkflowCircuitBreakerManager).to receive(:category_states).with('providers').and_return([
  ...
  describe 'POST #circuit_breakers_category_reset' do
    before do
      allow(WorkflowCircuitBreakerManager).to receive(:reset_category!)
      allow(WorkflowCircuitBreakerManager).to receive(:category_states).with('providers').and_return([])
    end
    ...
    context 'with valid permissions' do
      ...
      it 'resets circuit breakers in category' do
        expect(WorkflowCircuitBreakerManager).to receive(:reset_category!).with('providers')
  ...
  describe 'GET #circuit_breakers_monitor' do
    before do
      allow(WorkflowCircuitBreakerManager).to receive(:monitor_and_alert).and_return({
        total: 5,
        healthy: 4,
        degraded: 1,
        unhealthy: 0
    ...
    context 'with valid permissions' do
      ...
      it 'monitors circuit breakers and returns alerts' do
        expect(WorkflowCircuitBreakerManager).to receive(:monitor_and_alert)
  ...
  describe 'worker authentication' do
    before do
      ...
      allow(WorkflowCircuitBreakerManager).to receive(:all_states).and_return([])
      allow(WorkflowCircuitBreakerManager).to receive(:health_summary).and_return({ total: 0 })
    end
