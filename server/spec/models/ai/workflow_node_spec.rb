...
RSpec.describe Ai::WorkflowNode, type: :model do
  describe 'associations' do
    it { should belong_to(:workflow) }
    it { should have_many(:node_executions).dependent(:destroy) }
    it { should have_many(:source_edges).class_name('Ai::WorkflowEdge').with_foreign_key('source_node_id').with_primary_key('node_id') }
    it { should have_many(:target_edges).class_name('Ai::WorkflowEdge').with_foreign_key('target_node_id').with_primary_key('node_id') }
  end
    ...
    context 'node_id uniqueness' do
      ...
      it 'validates uniqueness of node_id within workflow scope' do
        duplicate_node = build(:ai_workflow_node,
                              ...
                              workflow: existing_node.workflow)
    ...
    context 'configuration validation' do
      context 'ai_agent node type' do
        it 'requires agent_id in configuration' do
          ...
          node = build(:ai_workflow_node, node_type: 'ai_agent', workflow: workflow)
      ...
      context 'sub_workflow node type' do
        ...
        let(:node) { build(:ai_workflow_node, :sub_workflow, workflow: workflow) }
  ...
  describe 'scopes' do
    let(:workflow) { create(:ai_workflow) }
    let!(:start_node) { create(:ai_workflow_node, :start_node, workflow: workflow) }
    let!(:ai_agent_node) { create(:ai_workflow_node, :ai_agent, workflow: workflow) }
    let!(:end_node) { create(:ai_workflow_node, :end_node, workflow: workflow) }
    let!(:error_handler_node) { create(:ai_workflow_node, :condition, workflow: workflow, is_error_handler: true) }

    describe '.by_type' do
      it 'filters nodes by type' do
        ai_agents = Ai::WorkflowNode.by_type('ai_agent')
    ...
    describe '.start_nodes' do
      it 'returns only start nodes' do
        start_nodes = Ai::WorkflowNode.start_nodes
    ...
    describe '.end_nodes' do
      it 'returns only end nodes' do
        end_nodes = Ai::WorkflowNode.end_nodes
    ...
    describe '.error_handlers' do
      it 'returns nodes marked as error handlers' do
        error_handlers = Ai::WorkflowNode.error_handlers
    ...
    describe '.ordered_by_position' do
      it 'orders nodes by created_at' do
        ordered = workflow.nodes.ordered_by_position
  ...
  describe 'callbacks' do
    ...
    describe 'after_destroy' do
      it 'removes associated edges' do
        ...
        other_node = create(:ai_workflow_node, workflow: workflow)
        ...
        expect { node.destroy }.to change { Ai::WorkflowEdge.count }.by(-1)
      end
    end
  end
  ...
  describe 'instance methods' do
    ...
    describe '#next_nodes' do
      ...
      let(:next_node1) { create(:ai_workflow_node, workflow: node.workflow) }
      let(:next_node2) { create(:ai_workflow_node, workflow: node.workflow) }
    ...
    describe '#previous_nodes' do
      let(:node) { create(:ai_workflow_node) }
      let(:prev_node1) { create(:ai_workflow_node, workflow: node.workflow) }
      let(:prev_node2) { create(:ai_workflow_node, workflow: node.workflow) }
    ...
    describe '#has_conditions?' do
      let(:node) { create(:ai_workflow_node) }
      let(:other_node) { create(:ai_workflow_node, workflow: node.workflow) }
    ...
    describe '#execution_summary' do
      ...
      before do
        ...
        3.times do |i|
          run = create(:ai_workflow_run, workflow: node.workflow, account: node.workflow.account)
          status = i < 2 ? 'completed' : 'failed'
          duration = i < 2 ? 1000 + (i * 1000) : nil
          create(:ai_workflow_node_execution, node: node, workflow_run: run, status: status, duration_ms: duration)
        end
      end
    ...
    describe '#distance_to' do
      ...
      let(:node2) { create(:ai_workflow_node, workflow: node1.workflow, position: { 'x' => 3, 'y' => 4 }) }
