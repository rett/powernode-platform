...
RSpec.describe Ai::WorkflowEdge, type: :model do
  describe 'associations' do
    it { should belong_to(:workflow) }
    it { should belong_to(:source_node).class_name('Ai::WorkflowNode').with_foreign_key('source_node_id').with_primary_key('node_id') }
    it { should belong_to(:target_node).class_name('Ai::WorkflowNode').with_foreign_key('target_node_id').with_primary_key('node_id') }
  end
  ...
  describe 'validations' do
    ...
    context 'edge_id uniqueness' do
      ...
      it 'validates uniqueness of edge_id within workflow scope' do
        workflow = existing_edge.workflow
        node1 = create(:ai_workflow_node, workflow: workflow)
        node2 = create(:ai_workflow_node, workflow: workflow)
      ...
      it 'allows same edge_id in different workflows' do
        ...
        node1_w1 = create(:ai_workflow_node, workflow: workflow1)
        node2_w1 = create(:ai_workflow_node, workflow: workflow1)
        node1_w2 = create(:ai_workflow_node, workflow: workflow2)
        node2_w2 = create(:ai_workflow_node, workflow: workflow2)
    ...
    context 'start/end node connection validation' do
      ...
      let(:regular_node) { create(:ai_workflow_node, workflow: workflow) }
  ...
  describe 'scopes' do
    let(:workflow) { create(:ai_workflow) }
    let(:node1) { create(:ai_workflow_node, workflow: workflow) }
    let(:node2) { create(:ai_workflow_node, workflow: workflow) }
    let(:node3) { create(:ai_workflow_node, workflow: workflow) }

    let!(:default_edge) { create(:ai_workflow_edge, workflow: workflow, source_node_id: node1.node_id, target_node_id: node2.node_id, edge_type: 'default') }
    let!(:success_edge) { create(:ai_workflow_edge, workflow: workflow, source_node_id: node1.node_id, target_node_id: node3.node_id, edge_type: 'success') }
    let!(:error_edge) { create(:ai_workflow_edge, workflow: workflow, source_node_id: node2.node_id, target_node_id: node3.node_id, edge_type: 'error') }
    let!(:conditional_edge) { create(:ai_workflow_edge, :conditional, workflow: workflow, source_node_id: node3.node_id, target_node_id: node1.node_id) }
  ...
  describe 'instance methods' do
    ...
    describe '#creates_cycle?' do
      ...
      it 'detects cycles' do
        ...
        edge = build(:ai_workflow_edge, workflow: workflow,
      ...
      it 'returns false when no cycle' do
        node4 = create(:ai_workflow_node, workflow: workflow)
        edge = build(:ai_workflow_edge, workflow: workflow,
