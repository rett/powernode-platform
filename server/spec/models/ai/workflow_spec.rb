...
RSpec.describe Ai::Workflow, type: :model do
    ...
    describe '#execute' do
      ...
      context 'with valid execution parameters' do
        ...
        it 'returns the created workflow run' do
          ...
          expect(run).to be_a(Ai::WorkflowRun)
  ...
  describe 'workflow structure validation' do
    describe '#has_valid_structure?' do
      ...
      it 'returns false for workflow without start node' do
        workflow = create(:ai_workflow)
        create(:ai_workflow_node, workflow: workflow, is_start_node: false)
    ...
    describe '#validate_structure' do
      ...
      it 'returns errors for workflow without start node' do
        workflow = create(:ai_workflow)
        create(:ai_workflow_node, workflow: workflow, is_start_node: false, is_end_node: true)
      ...
      it 'returns warnings for workflow without end nodes' do
        workflow = create(:ai_workflow)
        create(:ai_workflow_node, workflow: workflow, is_start_node: true, is_end_node: false)
    ...
    describe '#start_nodes' do
      it 'returns nodes marked as start nodes' do
        workflow = create(:ai_workflow)
        start_node = create(:ai_workflow_node, workflow: workflow, is_start_node: true)
        create(:ai_workflow_node, workflow: workflow, is_start_node: false)
    ...
    describe '#end_nodes' do
      it 'returns nodes marked as end nodes' do
        workflow = create(:ai_workflow)
        end_node = create(:ai_workflow_node, workflow: workflow, is_end_node: true)
        create(:ai_workflow_node, workflow: workflow, is_end_node: false)
    ...
    describe '#node_count' do
      it 'returns count of workflow nodes' do
        workflow = create(:ai_workflow)
        create_list(:ai_workflow_node, 3, workflow: workflow)
  ...
  describe 'state management' do
    ...
    describe '#archive!' do
      ...
      it 'deactivates related schedules' do
        schedule = create(:ai_workflow_schedule, workflow: workflow, is_active: true, status: 'active')
        workflow.archive!
  ...
  describe 'duplication' do
    ...
    describe '#duplicate_for_account' do
      ...
      it 'creates workflow in target account' do
        ...
        start_node = create(:ai_workflow_node, workflow: original, node_type: 'start', is_start_node: true, configuration: {})
        end_node = create(:ai_workflow_node, workflow: original, node_type: 'end', is_end_node: true, configuration: {})
        create(:ai_workflow_edge, workflow: original, source_node_id: start_node.node_id, target_node_id: end_node.node_id)
  ...
  describe 'statistics and metrics' do
    ...
    describe '#execution_stats' do
      before do
        create_list(:ai_workflow_run, 3, :completed, workflow: workflow)
    ...
    describe '#recent_runs' do
      it 'returns runs from specified period' do
        old_run = create(:ai_workflow_run, workflow: workflow, created_at: 2.days.ago)
        recent_run = create(:ai_workflow_run, workflow: workflow, created_at: 1.hour.ago)
    ...
    describe '#total_cost' do
      it 'sums cost from completed runs' do
        ...
        workflow.runs.update_all(total_cost: 0.05)
    ...
    describe '#average_execution_time' do
      it 'calculates average from completed runs' do
        create(:ai_workflow_run, :completed, workflow: workflow, duration_ms: 1000)
        create(:ai_workflow_run, :completed, workflow: workflow, duration_ms: 2000)
  ...
  describe 'edge cases' do
    ...
    it 'prevents deletion with recent runs' do
      workflow = create(:ai_workflow)
      create(:ai_workflow_run, workflow: workflow, created_at: 1.minute.ago)
