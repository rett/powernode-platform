...
RSpec.describe GitPipelineApproval, type: :model do
  ...
  describe 'associations' do
    ...
    it { is_expected.to have_one(:git_repository).through(:git_pipeline) }
  end
  ...
  describe 'validations' do
    ...
    it 'validates status inclusion' do
      GitPipelineApproval::STATUSES.each do |status|
    ...
    it 'validates gate_name uniqueness scoped to pipeline' do
      ...
      duplicate = build(:git_pipeline_approval,
                        git_pipeline: existing.git_pipeline,
  ...
  describe 'scopes' do
    ...
    let!(:rejected_approval) { create(:git_pipeline_approval, :rejected, git_pipeline: pipeline, account: account) }
    let!(:expired_approval) { create(:git_pipeline_approval, :expired, git_pipeline: pipeline, account: account) }
    let!(:cancelled_approval) { create(:git_pipeline_approval, :cancelled, git_pipeline: pipeline, account: account) }
    ...
    describe '.active' do
      let!(:active_approval) { create(:git_pipeline_approval, status: 'pending', expires_at: 1.hour.from_now, git_pipeline: pipeline, account: account) }
      let!(:expired_pending) { create(:git_pipeline_approval, status: 'pending', expires_at: 1.hour.ago, git_pipeline: pipeline, account: account) }
    ...
    describe '.expiring_soon' do
      let!(:expiring_approval) { create(:git_pipeline_approval, :expiring_soon, git_pipeline: pipeline, account: account) }
    ...
    describe '.for_environment' do
      let!(:prod_approval) { create(:git_pipeline_approval, :production, git_pipeline: pipeline, account: account) }
      let!(:staging_approval) { create(:git_pipeline_approval, :staging, git_pipeline: pipeline, account: account) }
  ...
  describe 'status transitions' do
    ...
    describe '#approve!' do
      let(:approval) { create(:git_pipeline_approval, :pending, git_pipeline: pipeline, account: account, expires_at: 1.hour.from_now) }
      ...
      it 'returns false if cannot respond' do
        expired_approval = create(:git_pipeline_approval, :pending, git_pipeline: pipeline, account: account, expires_at: 1.hour.ago)
    ...
    describe '#reject!' do
      ...
      it 'returns false if cannot respond' do
        already_approved = create(:git_pipeline_approval, :approved, git_pipeline: pipeline, account: account)
    ...
    describe '#expire!' do
      ...
      it 'returns false if not pending' do
        approved = create(:git_pipeline_approval, :approved, git_pipeline: pipeline, account: account)
  ...
  describe '#can_user_approve?' do
    ...
    context 'when no required_approvers' do
      let(:approval) { create(:git_pipeline_approval, :pending, git_pipeline: pipeline, account: account, required_approvers: []) }
    ...
    context 'when user is in required_approvers' do
      let(:approval) { create(:git_pipeline_approval, :pending, git_pipeline: pipeline, account: account, required_approvers: [user.id]) }
    ...
    context 'when user is not in required_approvers' do
      ...
      let(:approval) { create(:git_pipeline_approval, :pending, git_pipeline: pipeline, account: account, required_approvers: [other_user.id]) }
    ...
    context 'when approval is not pending' do
      let(:approval) { create(:git_pipeline_approval, :approved, git_pipeline: pipeline, account: account) }
  ...
  describe 'class methods' do
    ...
    describe '.expire_stale!' do
      let!(:stale_approval) { create(:git_pipeline_approval, :pending, git_pipeline: pipeline, account: account, expires_at: 1.hour.ago) }
      let!(:fresh_approval) { create(:git_pipeline_approval, :pending, git_pipeline: pipeline, account: account, expires_at: 1.hour.from_now) }
    ...
    describe '.stats_for_account' do
      before do
        create(:git_pipeline_approval, :pending, git_pipeline: pipeline, account: account)
        create(:git_pipeline_approval, :approved, git_pipeline: pipeline, account: account)
        create(:git_pipeline_approval, :rejected, git_pipeline: pipeline, account: account)
        create(:git_pipeline_approval, :expired, git_pipeline: pipeline, account: account)
      end
