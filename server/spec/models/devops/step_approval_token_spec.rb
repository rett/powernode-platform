...
RSpec.describe Devops::StepApprovalToken, type: :model do
  ...
  describe 'associations' do
    it { is_expected.to belong_to(:step_execution).class_name('Devops::StepExecution') }
  ...
  describe 'validations' do
    subject { build(:devops_step_approval_token, step_execution: step_execution) }
    ...
    it 'auto-sets expires_at if not provided via callback' do
      token = build(:devops_step_approval_token, step_execution: step_execution, expires_at: nil)
      token.valid?
      expect(token.expires_at).to be_present
    end

    it 'validates email format' do
      token = build(:devops_step_approval_token,
    ...
    it 'validates token_digest uniqueness' do
      existing_token = create(:devops_step_approval_token, step_execution: step_execution)
      duplicate_token = build(:devops_step_approval_token,
                              step_execution: step_execution,
  ...
  describe 'scopes' do
    ...
    let(:third_step) { create(:devops_pipeline_step, :with_approval, pipeline: pipeline, name: 'Third Step') }
  ...
  describe '#expire!' do
    let!(:token) { create(:devops_step_approval_token, step_execution: step_execution) }
  ...
  describe '#pending?' do
    it 'returns true for pending status' do
      token = build(:devops_step_approval_token, status: 'pending')
      expect(token.pending?).to be true
    end

    it 'returns false for non-pending status' do
      token = build(:devops_step_approval_token, status: 'approved')
  ...
  describe '#rejected?' do
    it 'returns true for rejected status' do
      token = build(:devops_step_approval_token, status: 'rejected')
  ...
  describe 'callbacks' do
    describe '#set_default_expiry' do
      it 'sets expiry based on step approval_settings' do
        step_with_custom_timeout = create(:devops_pipeline_step,
                                           pipeline: pipeline,
                                           name: 'Custom Timeout Step',
                                           requires_approval: true,
                                           approval_settings: { 'timeout_hours' => 48 })
        execution_with_custom = create(:devops_step_execution,
                                        :waiting_approval,
                                        pipeline_run: pipeline_run,
      ...
      it 'defaults to 24 hours if no approval_settings' do
        step_without_timeout = create(:devops_pipeline_step,
                                       pipeline: pipeline,
                                       name: 'No Timeout Step',
                                       requires_approval: true,
                                       approval_settings: {})
        execution_without = create(:devops_step_execution,
                                    :waiting_approval,
                                    pipeline_run: pipeline_run,
