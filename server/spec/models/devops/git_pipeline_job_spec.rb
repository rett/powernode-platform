...
RSpec.describe Devops::GitPipelineJob, type: :model do
  ...
  describe 'scopes' do
    ...
    let!(:pending_job) { create(:git_pipeline_job, :pending, pipeline: pipeline, account: account) }
    let!(:running_job) { create(:git_pipeline_job, :running, pipeline: pipeline, account: account) }
    let!(:success_job) { create(:git_pipeline_job, :success, pipeline: pipeline, account: account) }
    let!(:failure_job) { create(:git_pipeline_job, :failure, pipeline: pipeline, account: account) }
    ...
    describe '.failed' do
      it 'returns failed status jobs' do
        failed = create(:git_pipeline_job, status: 'failed', pipeline: pipeline, account: account)
  ...
  describe 'status transitions' do
    ...
    describe '#start!' do
      let(:job) { create(:git_pipeline_job, :pending, pipeline: pipeline, account: account) }
    ...
    describe '#complete!' do
      let(:job) { create(:git_pipeline_job, :running, pipeline: pipeline, account: account) }
