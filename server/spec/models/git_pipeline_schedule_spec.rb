...
RSpec.describe GitPipelineSchedule, type: :model do
  subject(:schedule) { build(:git_pipeline_schedule) }

  describe 'associations' do
    it { is_expected.to belong_to(:git_repository) }
    it { is_expected.to belong_to(:account) }
    it { is_expected.to belong_to(:created_by).class_name('User').optional }
    it { is_expected.to belong_to(:last_pipeline).class_name('GitPipeline').optional }
    it { is_expected.to have_one(:git_provider_credential).through(:git_repository) }
    it { is_expected.to have_one(:git_provider).through(:git_repository) }
  end
  ...
  describe 'validations' do
    ...
    it 'validates name uniqueness scoped to repository' do
      ...
      duplicate = build(:git_pipeline_schedule,
                        git_repository: existing.git_repository,
                        name: existing.name)
  ...
  describe 'scopes' do
    ...
    let!(:active_schedule) { create(:git_pipeline_schedule, :active, git_repository: repo, account: account) }
    let!(:inactive_schedule) { create(:git_pipeline_schedule, :inactive, git_repository: repo, account: account) }
    ...
    describe '.due' do
      let!(:due_schedule) do
        schedule = create(:git_pipeline_schedule, :active, git_repository: repo, account: account)
    ...
    describe '.by_status' do
      let!(:successful_schedule) { create(:git_pipeline_schedule, :with_history, git_repository: repo, account: account) }
  ...
  describe 'callbacks' do
    describe 'before_create :calculate_next_run' do
      ...
      it 'sets next_run_at on creation' do
        schedule = create(:git_pipeline_schedule, git_repository: repo, account: account)
  ...
  describe 'instance methods' do
    ...
    describe '#activate!' do
      ...
      let(:credential) { create(:git_provider_credential, git_provider: provider, account: account) }
      let(:repo) { create(:git_repository, git_provider_credential: credential, account: account) }
      let(:schedule) { create(:git_pipeline_schedule, :inactive, :failing, git_repository: repo, account: account) }
    ...
    describe '#deactivate!' do
      ...
      let(:credential) { create(:git_provider_credential, git_provider: provider, account: account) }
      let(:repo) { create(:git_repository, git_provider_credential: credential, account: account) }
      let(:schedule) { create(:git_pipeline_schedule, :active, git_repository: repo, account: account) }
    ...
    describe '#record_run!' do
      ...
      let(:credential) { create(:git_provider_credential, git_provider: provider, account: account) }
      let(:repo) { create(:git_repository, git_provider_credential: credential, account: account) }
      let(:schedule) { create(:git_pipeline_schedule, run_count: 10, success_count: 8, failure_count: 2, consecutive_failures: 1, git_repository: repo, account: account) }
      ...
      it 'records pipeline reference when provided' do
        pipeline = create(:git_pipeline, git_repository: repo, account: account)
  ...
  describe 'class methods' do
    ...
    describe '.due_for_execution' do
      ...
      let!(:future_schedule) { create(:git_pipeline_schedule, :active, git_repository: repo, account: account) }
      let!(:inactive_schedule) do
        schedule = create(:git_pipeline_schedule, :inactive, git_repository: repo, account: account)
        schedule.update_column(:next_run_at, 1.hour.ago)
        schedule
      end
    ...
    describe '.with_failures' do
      let!(:failing_schedule) { create(:git_pipeline_schedule, :failing, git_repository: repo, account: account) }
      let!(:healthy_schedule) { create(:git_pipeline_schedule, consecutive_failures: 0, git_repository: repo, account: account) }
