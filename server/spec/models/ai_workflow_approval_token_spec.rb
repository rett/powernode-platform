...
RSpec.describe AiWorkflowApprovalToken, type: :model do
  let(:account) { create(:account) }
  let(:workflow) { create(:ai_workflow, account: account) }
  let(:workflow_run) { create(:ai_workflow_run, ai_workflow: workflow) }
  let(:workflow_node) { create(:ai_workflow_node, ai_workflow: workflow) }
  ...
  describe 'associations' do
    it { is_expected.to belong_to(:ai_workflow_node_execution) }
  ...
  describe 'validations' do
    subject { build(:ai_workflow_approval_token, ai_workflow_node_execution: node_execution) }
    ...
    it 'auto-sets expires_at if not provided' do
      token = build(:ai_workflow_approval_token, ai_workflow_node_execution: node_execution, expires_at: nil)
      token.valid?
    ...
    it 'validates token_digest uniqueness' do
      existing_token = create(:ai_workflow_approval_token, ai_workflow_node_execution: node_execution)
  ...
  describe 'scopes' do
    ...
    describe '.for_node_execution' do
      ...
      let(:other_workflow_node) { create(:ai_workflow_node, ai_workflow: workflow) }
      let(:other_execution) { create(:ai_workflow_node_execution, ai_workflow_run: workflow_run, ai_workflow_node: other_workflow_node) }
      let!(:other_token) { create(:ai_workflow_approval_token, ai_workflow_node_execution: other_execution) }
  ...
  describe '#expire!' do
    let!(:token) { create(:ai_workflow_approval_token, ai_workflow_node_execution: node_execution) }
