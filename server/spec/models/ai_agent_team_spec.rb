...
RSpec.describe AiAgentTeam, type: :model do
  # ==========================================
  # Associations
  # ==========================================
  describe 'associations' do
    it { should belong_to(:account) }
    it { should have_many(:ai_agent_team_members).dependent(:destroy) }
    it { should have_many(:ai_agents).through(:ai_agent_team_members) }
  end
  ...
  describe 'validations' do
    ...
    it { should validate_inclusion_of(:team_type).in_array(AiAgentTeam::TEAM_TYPES) }
    it { should validate_inclusion_of(:coordination_strategy).in_array(AiAgentTeam::COORDINATION_STRATEGIES) }
    it { should validate_inclusion_of(:status).in_array(AiAgentTeam::STATUSES) }
  ...
  describe 'scopes' do
    ...
    it 'filters active teams' do
      expect(AiAgentTeam.active).to include(active_team, hierarchical_team, mesh_team)
      expect(AiAgentTeam.active).not_to include(inactive_team, archived_team)
    end

    it 'filters inactive teams' do
      expect(AiAgentTeam.inactive).to include(inactive_team)
    end

    it 'filters archived teams' do
      expect(AiAgentTeam.archived).to include(archived_team)
    end

    it 'filters by team type' do
      expect(AiAgentTeam.hierarchical).to include(hierarchical_team, active_team)
      expect(AiAgentTeam.mesh).to include(mesh_team)
    end
  end
  ...
  describe '#ordered_members' do
    it 'returns members ordered by priority' do
      ...
      member3 = create(:ai_agent_team_member, ai_agent_team: team)
