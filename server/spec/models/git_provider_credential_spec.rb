...
RSpec.describe GitProviderCredential, type: :model do
  ...
  describe 'associations' do
    ...
    it { is_expected.to have_many(:git_repositories).dependent(:destroy) }
  end
  ...
  describe 'scopes' do
    ...
    describe '.active' do
      let!(:active_cred) { create(:git_provider_credential, git_provider: provider, account: account, is_active: true) }
      let!(:inactive_cred) { create(:git_provider_credential, git_provider: other_provider, account: account, is_active: false) }
    ...
    describe '.default' do
      let!(:first_cred) { create(:git_provider_credential, git_provider: provider, account: account) }
      let!(:non_default) { create(:git_provider_credential, git_provider: other_provider, account: account) }
    ...
    describe '.for_provider' do
      let!(:cred1) { create(:git_provider_credential, git_provider: provider, account: account) }
      let!(:cred2) { create(:git_provider_credential, git_provider: other_provider, account: account) }
    ...
    describe '.healthy' do
      let!(:healthy_cred) { create(:git_provider_credential, git_provider: provider, account: account, consecutive_failures: 0) }
      let!(:unhealthy_cred) { create(:git_provider_credential, git_provider: other_provider, account: account, consecutive_failures: 5) }
  ...
  describe 'instance methods' do
    ...
    describe '#record_success!' do
      let(:account) { create(:account) }
      let(:provider) { create(:git_provider) }
      let(:credential) { create(:git_provider_credential, git_provider: provider, account: account, consecutive_failures: 2, failure_count: 5) }
    ...
    describe '#record_failure!' do
      let(:account) { create(:account) }
      let(:provider) { create(:git_provider) }
      let(:credential) { create(:git_provider_credential, git_provider: provider, account: account, consecutive_failures: 0) }
  ...
  describe 'credential encryption' do
    ...
    describe 'credentials=' do
      it 'encrypts credentials when setting' do
        credential = build(:git_provider_credential, git_provider: provider, account: account, encrypted_credentials: nil, encryption_key_id: nil)
    ...
    describe '#credentials' do
      it 'decrypts stored credentials' do
        credential = create(:git_provider_credential, git_provider: provider, account: account)
  ...
  describe 'default management' do
    ...
    it 'sets first credential as default automatically' do
      cred = create(:git_provider_credential, git_provider: provider, account: account)
      expect(cred.reload.is_default).to be true
    end

    it 'ensures only one default per provider per account' do
      cred1 = create(:git_provider_credential, git_provider: provider, account: account)
      expect(cred1.reload.is_default).to be true

      cred2 = create(:git_provider_credential, git_provider: provider, account: account, is_default: false)
      cred2.make_default!
