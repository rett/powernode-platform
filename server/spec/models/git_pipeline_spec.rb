...
RSpec.describe GitPipeline, type: :model do
  subject(:pipeline) { build(:git_pipeline) }

  describe 'associations' do
    it { is_expected.to belong_to(:git_repository) }
    it { is_expected.to belong_to(:account) }
    it { is_expected.to have_many(:git_pipeline_jobs).dependent(:destroy) }
  end
  ...
  describe 'scopes' do
    ...
    let!(:pending_pipeline) { create(:git_pipeline, :pending, git_repository: repo, account: account) }
    let!(:running_pipeline) { create(:git_pipeline, :running, git_repository: repo, account: account) }
    let!(:success_pipeline) { create(:git_pipeline, :success, git_repository: repo, account: account) }
    let!(:failure_pipeline) { create(:git_pipeline, :failure, git_repository: repo, account: account) }
    ...
    describe '.failed' do
      it 'returns failed status pipelines' do
        failed = create(:git_pipeline, status: 'failed', git_repository: repo, account: account)
  ...
  describe 'status transitions' do
    ...
    describe '#start!' do
      let(:pipeline) { create(:git_pipeline, :pending, git_repository: repo, account: account) }
    ...
    describe '#complete!' do
      let(:pipeline) { create(:git_pipeline, :running, git_repository: repo, account: account) }
