...
RSpec.describe GitRepository, type: :model do
  ...
  describe 'associations' do
    ...
    it { is_expected.to have_many(:git_pipelines).dependent(:destroy) }
  ...
  describe 'validations' do
    ...
    context 'uniqueness' do
      ...
      before do
        create(:git_repository, git_provider_credential: credential, account: account, full_name: 'owner/repo1')
      end

      it 'validates full_name uniqueness per account' do
        duplicate = build(:git_repository, git_provider_credential: credential, account: account, full_name: 'owner/repo1')
  ...
  describe 'scopes' do
    ...
    let!(:public_repo) { create(:git_repository, git_provider_credential: credential, account: account, is_private: false) }
    let!(:private_repo) { create(:git_repository, :private, git_provider_credential: credential, account: account) }
    let!(:archived_repo) { create(:git_repository, :archived, git_provider_credential: credential, account: account) }
    let!(:webhook_repo) { create(:git_repository, :with_webhook, git_provider_credential: credential, account: account) }
  ...
  describe 'instance methods' do
    ...
    describe '#provider_type' do
      it 'returns the provider type from the credential' do
        repo = build(:git_repository, git_provider_credential: credential)
    ...
    describe '#mark_synced!' do
      let(:repo) { create(:git_repository, git_provider_credential: credential, account: account, last_synced_at: nil) }
    ...
    describe '#latest_pipeline' do
      ...
      it 'returns the most recent pipeline' do
        old_pipeline = create(:git_pipeline, git_repository: repo, account: account, created_at: 1.day.ago)
        new_pipeline = create(:git_pipeline, git_repository: repo, account: account, created_at: 1.hour.ago)
    ...
    describe '#pipeline_stats' do
      ...
      it 'returns pipeline statistics' do
        create(:git_pipeline, :success, git_repository: repo, account: account)
        create(:git_pipeline, :success, git_repository: repo, account: account)
        create(:git_pipeline, :failure, git_repository: repo, account: account)
  ...
  describe 'callbacks' do
    describe 'before_create' do
      ...
      it 'generates webhook_secret' do
        repo = create(:git_repository, git_provider_credential: credential, account: account, webhook_secret: nil)
