...
RSpec.describe AiAgent, 'MCP functionality', type: :model do
  ...
  describe 'MCP lifecycle callbacks' do
    let(:agent) { build(:ai_agent, account: account, creator: user, ai_provider: ai_provider, mcp_capabilities: ['text_generation']) }
  ...
  describe 'MCP scopes' do
    ...
    describe '.with_capability' do
      it 'returns agents with specific capabilities' do
        results = AiAgent.with_capability('text_generation')
