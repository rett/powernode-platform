...
RSpec.describe GitRunner, type: :model do
  subject(:runner) { build(:git_runner) }

  describe 'associations' do
    it { is_expected.to belong_to(:git_provider_credential) }
    it { is_expected.to belong_to(:git_repository).optional }
    it { is_expected.to belong_to(:account) }
    it { is_expected.to have_one(:git_provider).through(:git_provider_credential) }
  end
  ...
  describe 'validations' do
    ...
    it 'validates status inclusion' do
      GitRunner::STATUSES.each do |status|
    ...
    it 'validates runner_scope inclusion' do
      GitRunner::SCOPES.each do |scope|
    ...
    it 'validates external_id uniqueness scoped to credential' do
      ...
      duplicate = build(:git_runner,
                        git_provider_credential: existing.git_provider_credential,
  ...
  describe 'scopes' do
    ...
    let!(:online_runner) { create(:git_runner, :online, git_provider_credential: credential, account: account) }
    let!(:offline_runner) { create(:git_runner, :offline, git_provider_credential: credential, account: account) }
    let!(:busy_runner) { create(:git_runner, :busy, git_provider_credential: credential, account: account) }
    ...
    describe '.by_scope' do
      let!(:repo_runner) { create(:git_runner, runner_scope: 'repository', git_provider_credential: credential, account: account) }
      let!(:org_runner) { create(:git_runner, :organization_scope, git_provider_credential: credential, account: account) }
    ...
    describe '.recently_seen' do
      let!(:recent_runner) { create(:git_runner, last_seen_at: 1.minute.ago, git_provider_credential: credential, account: account) }
      let!(:stale_runner) { create(:git_runner, last_seen_at: 10.minutes.ago, git_provider_credential: credential, account: account) }
    ...
    describe '.stale' do
      let!(:recent_runner) { create(:git_runner, last_seen_at: 1.minute.ago, git_provider_credential: credential, account: account) }
      let!(:stale_runner) { create(:git_runner, last_seen_at: 10.minutes.ago, git_provider_credential: credential, account: account) }
      let!(:never_seen_runner) { create(:git_runner, last_seen_at: nil, git_provider_credential: credential, account: account) }
    ...
    describe '.with_label' do
      let!(:labeled_runner) { create(:git_runner, labels: ['linux', 'x64'], git_provider_credential: credential, account: account) }
  ...
  describe 'status updates' do
    ...
    describe '#mark_online!' do
      let(:runner) { create(:git_runner, :offline, git_provider_credential: credential, account: account) }
    ...
    describe '#mark_offline!' do
      let(:runner) { create(:git_runner, :online, git_provider_credential: credential, account: account) }
    ...
    describe '#mark_available!' do
      let(:runner) { create(:git_runner, :busy, git_provider_credential: credential, account: account) }
    ...
    describe '#record_job_completion!' do
      let(:runner) { create(:git_runner, total_jobs_run: 10, successful_jobs: 8, failed_jobs: 2, git_provider_credential: credential, account: account) }
    ...
    describe '#record_success!' do
      let(:runner) { create(:git_runner, total_jobs_run: 5, successful_jobs: 4, failed_jobs: 1, git_provider_credential: credential, account: account) }
  ...
  describe 'label management' do
    ...
    let(:provider) { create(:git_provider) }
    let(:credential) { create(:git_provider_credential, git_provider: provider, account: account) }
    let(:runner) { create(:git_runner, labels: ['linux', 'x64'], git_provider_credential: credential, account: account) }
  ...
  describe '.sync_from_provider' do
    ...
    it 'updates an existing runner' do
      existing = create(:git_runner, git_provider_credential: credential, account: account, external_id: '12345', name: 'old-name')
      runner = described_class.sync_from_provider(credential, runner_data)
