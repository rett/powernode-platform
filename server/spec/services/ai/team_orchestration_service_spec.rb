...
RSpec.describe Ai::TeamOrchestrationService, type: :service do
  ...
  describe '#get_team_analytics' do
    ...
    describe 'quality section - learnings' do
      ...
      it 'returns learning counts, categories, injections, and success rate' do
        ...
        # 1 with positive > negative out of 2 injected = 50%
    ...
    describe 'percentile edge cases' do
      ...
      it 'computes p95 correctly with two data points' do
        ...
        perf = service.get_team_analytics(team.id)[:performance]
        # With 2 values [1000, 9000], p95 index = round(0.95 * 1) = 1 -> 9000
  ...
  describe '#update_channel' do
    ...
    it 'raises for invalid participant_roles' do
      expect {
        service.update_channel(team.id, channel.id, { participant_roles: [SecureRandom.uuid] })
      }.to raise_error(ActiveRecord::RecordNotFound)
    end
  ...
  describe '#send_message' do
    ...
    context 'authority validation' do
      ...
      it 'raises RecordNotFound for invalid to_role_id' do
        expect {
          service.send_message(execution.id, { to_role_id: SecureRandom.uuid, content: 'Hi', message_type: 'task_update' })
        }.to raise_error(ActiveRecord::RecordNotFound)
      end
      ...
      it 'works with backward compatibility when roles absent' do
