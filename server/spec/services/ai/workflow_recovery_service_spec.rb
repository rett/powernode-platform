...
RSpec.describe Ai::WorkflowRecoveryService, type: :service do
  ...
  describe '#create_checkpoint' do
    ...
    before do
      ...
      workflow.nodes.first(2).each do |wf_node|
  ...
  describe 'recovery execution' do
    ...
    describe '#resume_from_checkpoint' do
      let(:resume_node) { workflow.nodes.find_by(node_type: 'start') } # Use start node which has outgoing edge
      let(:checkpoint) do
        {
          id: SecureRandom.uuid,
  ...
  describe 'error recovery strategies' do
    ...
    describe 'graceful degradation' do
      it 'skips failed non-critical nodes' do
        ...
        non_critical_node = workflow.nodes.find_by(
      ...
      it 'fails fast on critical node failures' do
        ...
        critical_node = workflow.nodes.find_by(
