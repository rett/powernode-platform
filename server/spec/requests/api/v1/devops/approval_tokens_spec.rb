...
RSpec.describe 'Api::V1::Devops::ApprovalTokens', type: :request do
  ...
  describe 'GET /api/v1/devops/approval_tokens/:token' do
    context 'with valid token' do
      it 'returns token details without authentication' do
        get "/api/v1/devops/approval_tokens/#{raw_token}"
    ...
    context 'with invalid token' do
      it 'returns not found' do
        get '/api/v1/devops/approval_tokens/invalid_token_12345'
  ...
  describe 'POST /api/v1/devops/approval_tokens/:token/approve' do
    before do
    ...
    context 'with valid pending token' do
      it 'approves the step without authentication' do
        post "/api/v1/devops/approval_tokens/#{raw_token}/approve",
      ...
      it 'calls handle_approval_response! on step execution' do
        expect_any_instance_of(Devops::StepExecution).to receive(:handle_approval_response!)
    ...
    context 'with comment required' do
      let(:pipeline_step_with_comment) do
        create(:devops_pipeline_step, :requires_comment, pipeline: pipeline, name: 'Comment Required Step')
      end
      ...
      let!(:token_with_comment_req) do
        token, @raw_token_comment = Devops::StepApprovalToken.create_for_recipient(
      ...
      it 'rejects approval without comment' do
        post "/api/v1/devops/approval_tokens/#{@raw_token_comment}/approve",
             as: :json
    ...
    context 'with invalid token' do
      it 'returns not found' do
        post '/api/v1/devops/approval_tokens/invalid_token/approve',
             as: :json
  ...
  describe 'POST /api/v1/devops/approval_tokens/:token/reject' do
    before do
      allow_any_instance_of(Devops::StepExecution).to receive(:handle_approval_response!).and_return(true)
    end
    ...
    context 'with valid pending token' do
      it 'rejects the step without authentication' do
        post "/api/v1/devops/approval_tokens/#{raw_token}/reject",
    ...
    context 'with comment required' do
      let(:pipeline_step_with_comment) do
        create(:devops_pipeline_step, :requires_comment, pipeline: pipeline, name: 'Reject Comment Step')
      end
      let(:step_execution_with_comment) do
        create(:devops_step_execution,
               :waiting_approval,
      ...
      let!(:token_with_comment_req) do
        token, @raw_token_reject = Devops::StepApprovalToken.create_for_recipient(
          step_execution: step_execution_with_comment,
          recipient_email: 'approver@example.com'
        )
        token
      ...
      it 'rejects without comment' do
        post "/api/v1/devops/approval_tokens/#{@raw_token_reject}/reject",
             as: :json
    ...
    context 'with invalid token' do
      it 'returns not found' do
        post '/api/v1/devops/approval_tokens/invalid_token/reject',
             as: :json
  ...
  describe 'authentication behavior' do
    ...
    it 'does not require JWT authentication for approve' do
      ...
      post "/api/v1/devops/approval_tokens/#{raw_token}/approve", as: :json
      expect(response).not_to have_http_status(:unauthorized)
    end
    ...
    it 'does not require JWT authentication for reject' do
      ...
      post "/api/v1/devops/approval_tokens/#{raw_token}/reject", as: :json
      expect(response).not_to have_http_status(:unauthorized)
    end
  end
