...
RSpec.describe 'Api::V1::SupplyChain::Attributions', type: :request do
  ...
  describe 'GET /api/v1/supply_chain/attributions' do
    ...
    let!(:attribution1) { create(:supply_chain_attribution, account: account, sbom: sbom, component: component, license: license) }
    let!(:attribution2) { create(:supply_chain_attribution, account: account, sbom: sbom) }
    ...
    context 'with proper permissions' do
      ...
      it 'filters by attribution type' do
        attribution1.update!(attribution_type: 'copyright')
        attribution2.update!(attribution_type: 'notice')

        get '/api/v1/supply_chain/attributions', params: { type: 'copyright' }, headers: headers

        expect_success_response
        data = json_response_data
        expect(data['attributions'].length).to eq(1)
        expect(data['attributions'].first['attribution_type']).to eq('copyright')
      end

      it 'filters by sbom_id' do
        get '/api/v1/supply_chain/attributions', params: { sbom_id: sbom.id }, headers: headers

        expect_success_response
        data = json_response_data
        expect(data['attributions'].all? { |a| a['sbom_id'] == sbom.id }).to be true
      end
  ...
  describe 'GET /api/v1/supply_chain/attributions/:id' do
    ...
    let(:component) { create(:supply_chain_component, account: account) }
    let(:license) { create(:supply_chain_license) }
    let(:attribution) { create(:supply_chain_attribution, account: account, sbom: sbom, component: component, license: license, created_by: user) }
    let(:other_attribution) { create(:supply_chain_attribution, account: other_account) }
    ...
    context 'with proper permissions' do
      it 'returns attribution details' do
        ...
        expect_success_response
        data = json_response_data
        expect(data['attribution']).to include(
          'id' => attribution.id,
          'attribution_type' => attribution.attribution_type,
          'sbom_id' => sbom.id
        )
        expect(data['attribution']['component']).to be_present
        expect(data['attribution']['license']).to be_present
        expect(data['attribution']['created_by']).to be_present
      end
  ...
  describe 'POST /api/v1/supply_chain/attributions' do
    ...
    let(:valid_params) do
      {
        attribution: {
          attribution_type: 'copyright',
          component_id: component.id,
          license_id: license.id,
          sbom_id: sbom.id,
          copyright_text: 'Copyright 2024 Test Company',
    ...
    context 'with proper permissions' do
      it 'creates a new attribution' do
        ...
        expect(account.supply_chain_attributions.last.created_by).to eq(user)
      end

      it 'returns validation errors for invalid params' do
        invalid_params = valid_params.deep_merge(attribution: { attribution_type: nil })
  ...
  describe 'PATCH /api/v1/supply_chain/attributions/:id' do
    let(:sbom) { create(:supply_chain_sbom, account: account) }
    let(:attribution) { create(:supply_chain_attribution, account: account, sbom: sbom) }
    let(:update_params) do
      {
        attribution: {
          copyright_text: 'Updated Copyright 2024',
    ...
    context 'with proper permissions' do
      it 'updates the attribution' do
        ...
        expect_success_response
        data = json_response_data
        expect(data['attribution']['copyright_text']).to eq('Updated Copyright 2024')
        expect(data['attribution']['attribution_text']).to eq('Updated attribution')
      end

      it 'returns validation errors for invalid update' do
        invalid_params = { attribution: { attribution_type: 'invalid' } }

        patch "/api/v1/supply_chain/attributions/#{attribution.id}", params: invalid_params, headers: headers, as: :json
  ...
  describe 'DELETE /api/v1/supply_chain/attributions/:id' do
    ...
    context 'with proper permissions' do
      it 'deletes the attribution' do
        ...
        expect_success_response
        expect(json_response_data['message']).to eq('Attribution deleted')
      end
    end
  ...
  describe 'POST /api/v1/supply_chain/attributions/generate_notice_file' do
    ...
    let!(:component) { create(:supply_chain_component, account: account) }
    let!(:license) { create(:supply_chain_license) }
    let!(:attribution) { create(:supply_chain_attribution, account: account, sbom: sbom, component: component, license: license) }
    ...
    context 'with proper permissions' do
      it 'generates a notice file' do
        ...
        expect(data['message']).to eq('Notice file generated')
      end
    ...
    context 'with proper permissions' do
      it 'exports attributions as JSON' do
        get '/api/v1/supply_chain/attributions/export',
            params: { format: 'json', sbom_id: sbom.id },
            headers: headers,
            as: :json
