...
RSpec.describe "Api::V1::BaaS::Customers", type: :request do
  ...
  describe "GET /api/v1/baas/customers" do
    ...
    it "paginates results" do
      get "/api/v1/baas/customers", params: { page: 1, per_page: 2 }, headers: headers
    ...
    it "returns 401 with invalid API key" do
      get "/api/v1/baas/customers", headers: { "Authorization" => "Bearer invalid" }

      expect(response).to have_http_status(:unauthorized)
    end
  end

  describe "GET /api/v1/baas/customers/:id" do
    let(:customer) { create(:baas_customer, baas_tenant: tenant) }

    it "returns the customer" do
      get "/api/v1/baas/customers/#{customer.external_id}", headers: headers
    ...
    it "returns 404 for non-existent customer" do
      get "/api/v1/baas/customers/nonexistent", headers: headers
    ...
    it "returns 404 for customer from different tenant" do
      ...
      get "/api/v1/baas/customers/#{other_customer.external_id}", headers: headers

      expect(response).to have_http_status(:not_found)
    end
  end

  describe "POST /api/v1/baas/customers" do
    let(:valid_params) do
      {
    ...
    it "creates a new customer" do
      expect {
        post "/api/v1/baas/customers", params: valid_params.to_json, headers: headers
    ...
    it "creates customer with auto-generated external_id when no params provided" do
      expect {
        post "/api/v1/baas/customers", params: {}.to_json, headers: headers
      }.to change { tenant.customers.count }.by(1)
    ...
    it "returns 422 for duplicate external_id" do
      ...
      post "/api/v1/baas/customers", params: { external_id: "existing-id" }.to_json, headers: headers

      expect(response).to have_http_status(:unprocessable_content)
    end
  end

  describe "PATCH /api/v1/baas/customers/:id" do
    let(:customer) { create(:baas_customer, baas_tenant: tenant) }

    it "updates the customer" do
      patch "/api/v1/baas/customers/#{customer.external_id}",
  ...
  describe "DELETE /api/v1/baas/customers/:id" do
    let!(:customer) { create(:baas_customer, baas_tenant: tenant) }

    it "archives the customer" do
      delete "/api/v1/baas/customers/#{customer.external_id}", headers: headers
