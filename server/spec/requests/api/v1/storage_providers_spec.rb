...
RSpec.describe 'Api::V1::StorageProviders', type: :request do
  ...
  describe 'GET /api/v1/storage' do
    ...
    context 'with proper permissions' do
      it 'returns list of storage providers' do
        get '/api/v1/storage', headers: headers, as: :json
  ...
  describe 'GET /api/v1/storage/:id' do
    let(:storage) { create(:file_storage, account: account) }

    context 'with proper permissions' do
      it 'returns storage provider details' do
        get "/api/v1/storage/#{storage.id}", headers: headers, as: :json
    ...
    context 'when storage not found' do
      it 'returns not found error' do
        get "/api/v1/storage/#{SecureRandom.uuid}", headers: headers, as: :json
  ...
  describe 'POST /api/v1/storage' do
    let(:valid_params) do
      {
        name: 'Test Storage',
        provider_type: 'local',
        configuration: { root_path: '/tmp/storage' }
      }
    end
    ...
    context 'with valid params' do
      it 'creates a new storage provider' do
        expect {
          post '/api/v1/storage', params: valid_params, headers: headers, as: :json
      ...
      it 'initializes storage if requested' do
        allow_any_instance_of(FileManagement::Storage).to receive_message_chain(:storage_provider, :initialize_storage)

        post '/api/v1/storage', params: valid_params.merge(initialize: true), headers: headers, as: :json
    ...
    context 'with invalid params' do
      it 'returns validation error' do
        ...
        post '/api/v1/storage', params: invalid_params, headers: headers, as: :json
  ...
  describe 'PATCH /api/v1/storage/:id' do
    let(:storage) { create(:file_storage, account: account) }
    ...
    context 'with valid params' do
      it 'updates the storage provider' do
        patch "/api/v1/storage/#{storage.id}", params: update_params, headers: headers, as: :json
  ...
  describe 'DELETE /api/v1/storage/:id' do
    let!(:storage) { create(:file_storage, account: account) }

    context 'when storage can be deleted' do
      it 'deletes the storage provider' do
        allow_any_instance_of(FileManagement::Storage).to receive(:files_count).and_return(0)
    ...
    context 'when storage is default' do
      ...
      it 'returns error' do
        delete "/api/v1/storage/#{default_storage.id}", headers: headers, as: :json
    ...
    context 'when storage has existing files' do
      it 'returns error' do
        allow_any_instance_of(FileManagement::Storage).to receive(:files_count).and_return(5)
  ...
  describe 'POST /api/v1/storage/:id/test' do
    ...
    context 'when connection succeeds' do
      it 'returns success response' do
        storage_record = storage # force factory evaluation before stubbing
        allow_any_instance_of(FileManagement::Storage).to receive_message_chain(:storage_provider, :test_connection).and_return({
          success: true,
          message: 'Connection successful'
        })

        post "/api/v1/storage/#{storage_record.id}/test", headers: headers, as: :json
  ...
  describe 'GET /api/v1/storage/:id/health' do
    let(:storage) { create(:file_storage, account: account) }

    it 'returns health check status' do
      storage_record = storage # force factory evaluation before stubbing
      allow_any_instance_of(FileManagement::Storage).to receive_message_chain(:storage_provider, :health_check).and_return({
        status: 'healthy',
        details: { available: true }
      })

      get "/api/v1/storage/#{storage_record.id}/health", headers: headers, as: :json
  ...
  describe 'POST /api/v1/storage/:id/set_default' do
    let!(:storage) { create(:file_storage, account: account) }
    let!(:default_storage) { create(:file_storage, account: account, is_default: true) }

    it 'sets storage as default' do
      post "/api/v1/storage/#{storage.id}/set_default", headers: headers, as: :json
  ...
  describe 'GET /api/v1/storage/supported' do
    it 'returns list of supported providers' do
      ...
      get '/api/v1/storage/supported', headers: headers, as: :json
  ...
  describe 'POST /api/v1/storage/:id/initialize' do
    let(:storage) { create(:file_storage, account: account) }

    it 'initializes storage backend' do
      allow_any_instance_of(FileManagement::Storage).to receive_message_chain(:storage_provider, :initialize_storage).and_return(true)

      post "/api/v1/storage/#{storage.id}/initialize", headers: headers, as: :json
  ...
  describe 'GET /api/v1/storage/:id/files' do
    let(:storage) { create(:file_storage, account: account) }
    ...
    it 'lists files in storage' do
      ...
      allow_any_instance_of(FileManagement::Storage).to receive_message_chain(:storage_provider, :list_files).and_return(files)

      get "/api/v1/storage/#{storage_record.id}/files", headers: headers, as: :json
  ...
  describe 'GET /api/v1/storage/stats' do
    let!(:storage1) { create(:file_storage, account: account) }
    let!(:storage2) { create(:file_storage, account: account) }

    it 'returns aggregate storage statistics' do
      allow_any_instance_of(FileManagement::Storage).to receive(:files_count).and_return(10)
      allow_any_instance_of(FileManagement::Storage).to receive(:total_size_bytes).and_return(1024)
      allow_any_instance_of(FileManagement::Storage).to receive(:quota_bytes).and_return(10240)
      allow_any_instance_of(FileManagement::Storage).to receive(:quota_enabled?).and_return(true)

      get '/api/v1/storage/stats', headers: headers, as: :json
