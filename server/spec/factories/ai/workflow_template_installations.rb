...
FactoryBot.define do
  factory :ai_workflow_template_installation, class: "Ai::WorkflowTemplateInstallation" do
    association :template, factory: :ai_workflow_template
    ...
    trait :with_created_workflow do
      after(:create) do |installation|
        workflow = create(:ai_workflow,
                         ...
                         name: "#{installation.template.name} (Installed)",
        ...
        installation.update!(workflow: workflow)
      end
    end

    trait :enterprise_template do
      template { create(:ai_workflow_template, :complex) }
      customizations do
        {
          workflow: {
            configuration: {
    ...
    trait :data_processing_template do
      template { create(:ai_workflow_template, :data_processing) }
      customizations do
        {
          workflow: {
