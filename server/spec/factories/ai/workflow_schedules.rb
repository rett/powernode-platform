...
FactoryBot.define do
  factory :ai_workflow_schedule, class: "Ai::WorkflowSchedule" do
    association :workflow, factory: :ai_workflow
    created_by { association :user, account: workflow.account }
    ...
    trait :daily do
      ...
      name { "Daily #{workflow.name}" }
      timezone { 'America/New_York' }
    end

    trait :weekly do
      cron_expression { '0 9 * * 1' } # Monday at 9 AM
      name { "Weekly #{workflow.name}" }
      configuration do
        {
          max_missed_runs: 2,
          allow_concurrent_runs: false,
    ...
    trait :weekdays_only do
      ...
      name { "Weekday #{workflow.name}" }
      timezone { 'America/Los_Angeles' }
    end

    trait :custom_interval do
      cron_expression { '*/15 * * * *' } # Every 15 minutes
      name { "Custom Interval #{workflow.name}" }
      configuration do
        {
          max_missed_runs: 10,
    ...
    trait :high_frequency do
      ...
      name { "High Frequency #{workflow.name}" }
      configuration do
        {
          max_missed_runs: 60,
          allow_concurrent_runs: true,
    ...
    trait :business_hours do
      ...
      name { "Business Hours #{workflow.name}" }
      timezone { 'America/New_York' }
      configuration do
        {
          holiday_calendar: 'US',
