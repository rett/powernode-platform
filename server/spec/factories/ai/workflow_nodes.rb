...
FactoryBot.define do
  factory :ai_workflow_node, class: "Ai::WorkflowNode" do
    ...
    after(:build) do |node, evaluator|
      if node.node_type == 'ai_agent'
        ...
        if node.configuration['agent_id'].blank? || !node.workflow.account.ai_agents.exists?(id: node.configuration['agent_id'])
          # Create minimal agent for this workflow's account - let factory generate unique name
          agent = create(:ai_agent, account: node.workflow.account)
