...
puts "\nTest 3: Agent configuration"
sample_agent = AiAgent.where(agent_type: 'workflow_operations').first
...
puts "\nTest 4: Agent capabilities"
agents_with_capabilities = AiAgent.where.not(capabilities: [])
puts "  Agents with capabilities: #{agents_with_capabilities.count}/#{AiAgent.count}"
if agents_with_capabilities.count == AiAgent.count
...
puts "\nTest 5: Agent execution readiness"
ready_agents = AiAgent.all.select { |agent| agent.can_execute? }
puts "  Ready for execution: #{ready_agents.count}/#{AiAgent.count}"
...
puts "\nTest 6: Agent metadata and versioning"
agents_with_metadata = AiAgent.where.not(metadata: {})
puts "  Agents with metadata: #{agents_with_metadata.count}/#{AiAgent.count}"
...
puts "\nTest 7: Account associations"
agents_with_accounts = AiAgent.joins(:account).count
puts "  Agents with accounts: #{agents_with_accounts}/#{AiAgent.count}"
if agents_with_accounts == AiAgent.count
...
puts "\nTest 8: Provider associations"
agents_with_providers = AiAgent.joins(:ai_provider).count
puts "  Agents with providers: #{agents_with_providers}/#{AiAgent.count}"
if agents_with_providers == AiAgent.count
...
puts "\n📊 Final Summary:"
puts "   Total Agents: #{AiAgent.count}"
puts "   Valid Agents: #{AiAgent.count - invalid_agents.count}"
puts "   Ready for Execution: #{ready_agents.count}"
puts "   Agent Types: #{AiAgent.distinct.count(:agent_type)}"
